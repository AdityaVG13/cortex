...
pub fn health_state_with_identity_fallback(status: u16, body: &str, expected_port: Option<u16>, expected_paths: Option<&ResolvedCortexPaths>) -> (bool, bool) {
...
pub fn probe_cortex_reachability_with_port(port: u16, timeout_ms: u64) -> CortexReachabilityProbe {
    ...
    let readiness_response = send_cortex_request_with_port(
        ...
        "",
        None,
        RequestTimeouts { connect: Duration::from_millis(timeout_ms), read: Duration::from_millis(timeout_ms), write: Duration::from_millis(timeout_ms) },
    );

    if let Ok(resp) = readiness_response {
        let (readiness_state, identity_mismatch) = readiness_state_with_identity_fallback(resp.status, &resp.body, Some(port), Some(&expected_paths));
        if let Some(ready) = readiness_state {
            return CortexReachabilityProbe { reachable: ready, starting: !ready, identity_mismatch };
        }
    }
    ...
    if let Ok(resp) = health_response {
        let (healthy, identity_mismatch) = health_state_with_identity_fallback(resp.status, &resp.body, Some(port), Some(&expected_paths));
        if healthy {
            return CortexReachabilityProbe { reachable: true, starting: false, identity_mismatch };
        }
    }
...
pub async fn wait_for_reachability(port: u16, target: bool, timeout: Duration) -> bool {
    tauri::async_runtime::spawn_blocking(move || wait_for_reachability_blocking(port, target, timeout)).await.unwrap_or(false)
}
...
pub async fn read_auth_token_with_retry() -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || read_auth_token_with_retry_blocking(Duration::from_millis(AUTH_TOKEN_WAIT_MS)))
        .await
...
pub fn cortex_readiness_state(status: u16, body: &str, expected_port: Option<u16>, expected_paths: Option<&ResolvedCortexPaths>) -> Option<bool> {
...
pub fn is_cortex_health_response(status: u16, body: &str, expected_port: Option<u16>, expected_paths: Option<&ResolvedCortexPaths>) -> bool {
    ...
    let runtime = json.get("runtime").and_then(|value| value.as_object());
    let stats = json.get("stats").and_then(|value| value.as_object());
    let runtime_port = runtime.and_then(|runtime| runtime.get("port")).and_then(|value| value.as_u64()).and_then(|value| u16::try_from(value).ok());
    ...
    if let Some(paths) = expected_paths {
        ...
        if !health_path_field_matches(runtime.and_then(|obj| obj.get("token_path")), paths.token.as_deref()) {
            return false;
        }
