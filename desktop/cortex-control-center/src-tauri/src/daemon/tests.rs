...
fn disallowed_daemon_binary_path_blocks_wrappers_temp_and_test_artifacts() {
    let wrapper = PathBuf::from("C:/repo/daemon-rs/target/debug/daemon-lifecycle-runtime/cortex-daemon-run.exe");
    ...
    let isolated_target_deps = PathBuf::from("C:/repo/daemon-rs/target-control-center-dev/debug/deps/cortex.exe");
    assert!(is_disallowed_daemon_binary_path(&isolated_target_deps));

    let shared_workspace_runtime = PathBuf::from(format!("C:/repo/daemon-rs/target/debug/{}", cortex_binary_file_name()));
    assert!(is_disallowed_daemon_binary_path(&shared_workspace_runtime));

    let isolated_runtime = PathBuf::from(format!("C:/repo/daemon-rs/target-control-center-dev/debug/{}", cortex_binary_file_name()));
    assert!(!is_disallowed_daemon_binary_path(&isolated_runtime));

    let isolated_release_runtime = PathBuf::from(format!("C:/repo/daemon-rs/target-control-center-release/release/{}", cortex_binary_file_name()));
