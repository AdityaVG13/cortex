...
use crate::cortex_http::readiness::{
    is_cortex_reachable_with_port, probe_cortex_reachability_with_port, wait_for_reachability_blocking, CortexReachabilityProbe,
};
...
pub fn try_service_ensure(port: u16) -> Result<bool, String> {
    ...
    let output = command.output().map_err(|err| format!("Failed to run `{}` service ensure: {err}", cortex_bin.display()))?;

    if !output.status.success() {
        return Err(format!("`cortex service ensure` failed: {}", command_output_summary(&output)));
    }
    ...
    Ok(wait_for_reachability_blocking(port, true, Duration::from_millis(SERVICE_ENSURE_WAIT_MS)))
}

pub fn try_local_app_managed_ensure(state: &DaemonState, port: u16) -> Result<CortexReachabilityProbe, String> {
    state.ensure_local_daemon()?;
    if wait_for_reachability_blocking(port, true, Duration::from_millis(LOCAL_DAEMON_START_WAIT_MS)) {
...
pub fn local_app_managed_start_timeout_message(state: &DaemonState, pid: Option<u32>, port: u16) -> String {
