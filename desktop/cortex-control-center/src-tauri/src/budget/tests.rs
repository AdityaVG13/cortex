...
"#,
    ...
    assert_eq!(snapshot.error.as_ref().map(|error| error.code.as_str()), None);
...
fn budget_editor_draft_serializes_only_enabled_endpoints() {
    let config = validate_budget_draft(BudgetConfigDraft {
        enabled: true,
        endpoints: vec![
            BudgetEndpointDraft { endpoint: "store".to_string(), enabled: false, limit: Some(120), window_seconds: Some(60) },
            BudgetEndpointDraft { endpoint: "recall".to_string(), enabled: true, limit: Some(42), window_seconds: Some(15) },
        ],
    })
...
fn budget_editor_rejects_duplicate_or_invalid_endpoint_drafts() {
    let duplicate = validate_budget_draft(BudgetConfigDraft {
        enabled: true,
        endpoints: vec![
            BudgetEndpointDraft { endpoint: "recall".to_string(), enabled: true, limit: Some(1), window_seconds: Some(60) },
            BudgetEndpointDraft { endpoint: "recall".to_string(), enabled: true, limit: Some(2), window_seconds: Some(60) },
        ],
    })
    .unwrap_err();
    ...
    let invalid = validate_budget_draft(BudgetConfigDraft {
        enabled: true,
        endpoints: vec![BudgetEndpointDraft { endpoint: "recall".to_string(), enabled: true, limit: Some(0), window_seconds: Some(60) }],
    })
    .unwrap_err();
