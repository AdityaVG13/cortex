...
pub fn write_dev_verification_report(content: String) -> Result<String, String> {
    ...
    let report_path =
        env::var("CORTEX_DEV_VERIFY_REPORT_PATH").map(PathBuf::from).map_err(|_| "CORTEX_DEV_VERIFY_REPORT_PATH is not configured.".to_string())?;

    if let Some(parent) = report_path.parent() {
        fs::create_dir_all(parent).map_err(|err| format!("Failed to create {}: {err}", parent.display()))?;
    }

    fs::write(&report_path, content).map_err(|err| format!("Failed to write {}: {err}", report_path.display()))?;
