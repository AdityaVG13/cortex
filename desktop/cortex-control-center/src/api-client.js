const TOKEN_REFRESH_ATTEMPTS = 4,
  TOKEN_REFRESH_DELAY_MS = 250,
  IPC_ABORT_TIMEOUT_MS = 8e3,
  IPC_ABORT_TIMEOUT_HEALTH_MS = 12e3,
  IPC_ABORT_TIMEOUT_MCP_MS = 3e4,
  IPC_ABORT_TIMEOUT_RECALL_MS = 2e4,
  IPC_ABORT_TIMEOUT_CORE_MS = 15e3,
  IPC_ABORT_TIMEOUT_SECONDARY_MS = 2e4,
  IPC_ABORT_TIMEOUT_ANALYTICS_MS = 6e4,
  IPC_TRANSPORT_MARGIN_MS = 500;
function wait(ms) {
...
function resolveIpcTransportTimeoutMs(path) {
  return Math.max(500, resolveIpcTimeoutMs(path) - 500);
}
function isIpcResponseEnvelope(value) {
  return !value || typeof value != "object"
    ? !1
    : typeof value.status == "number" && typeof value.body == "string";
}
...
function buildFallbackFailure(ipcError, httpError) {
  const ipcMessage = errorMessage(ipcError),
    httpMessage = errorMessage(httpError);
...
async function refreshTokenIfChanged(onTokenRefresh, getToken, previousToken) {
  if (!onTokenRefresh) return !1;
  const requiresRotation = !!previousToken;
  for (let attempt = 1; attempt <= 4; attempt += 1) {
    await onTokenRefresh(previousToken, attempt);
    const nextToken = getToken();
    if (!!nextToken && (!requiresRotation || nextToken !== previousToken))
      return !0;
    attempt < 4 && (await wait(250 * attempt));
  }
  return !1;
}
function createApi({ getInvoke, getToken, cortexBase, onTokenRefresh }) {
  return async function api(path, withAuth = !1, _retried = !1) {
    const invoke = getInvoke();
    let token = getToken();
    if (withAuth && !token && !_retried) {
      if (await refreshTokenIfChanged(onTokenRefresh, getToken, token))
        return api(path, withAuth, !0);
      token = getToken();
    }
    if (withAuth && !token)
      throw new Error(
        `${path}: no auth token (Tauri IPC ${invoke ? "available" : "missing"})`,
      );
    const requestViaHttp = async () => {
      ...
      if (
        ...
        withAuth &&
        !_retried &&
        (await refreshTokenIfChanged(onTokenRefresh, getToken, token))
      )
        return api(path, withAuth, !0);
      if (!response.ok) {
    ...
    if (invoke)
      try {
        const timeoutMs = resolveIpcTimeoutMs(path),
          transportTimeoutMs = resolveIpcTransportTimeoutMs(path),
        ...
        if (response.status < 200 || response.status >= 300)
          throw new Error(
            formatHttpError(path, response.status, response.body),
          );
        return JSON.parse(response.body);
      } catch (ipcError) {
        if (!shouldFallbackToHttp(ipcError)) throw ipcError;
        try {
...
function createPostApi({ getInvoke, getToken, cortexBase, onTokenRefresh }) {
  return async function postApi(path, body = {}, _retried = !1) {
    const invoke = getInvoke();
    let token = getToken();
    if (!token && !_retried) {
      if (await refreshTokenIfChanged(onTokenRefresh, getToken, token))
    ...
    const requestViaHttp = async () => {
      ...
      if (!response.ok) {
        const bodyText = await response.text().catch(() => "");
        throw new Error(
          formatHttpError(`POST ${path}`, response.status, bodyText),
        );
      }
      return await response.json();
    };
    if (invoke)
      try {
        ...
        if (response.status < 200 || response.status >= 300)
          throw new Error(
            formatHttpError(`POST ${path}`, response.status, response.body),
          );
        return JSON.parse(response.body);
      } catch (ipcError) {
        if (!shouldFallbackToHttp(ipcError)) throw ipcError;
...
function panelLabelFromError(message) {
  const path = String(message || "").split(":")[0],
    normalized = Object.keys(PANEL_LABELS).find((candidate) =>
...
function summarizeDashboardErrors(errors) {
...
async function settledWithRethrow(tasks) {
...
async function settledCollectErrors(fns) {
  const failures = (await Promise.allSettled(fns.map((fn) => fn()))).filter(
    (r) => r.status === "rejected",
  );
