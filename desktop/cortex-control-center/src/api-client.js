const TOKEN_REFRESH_ATTEMPTS=4,TOKEN_REFRESH_DELAY_MS=250,IPC_ABORT_TIMEOUT_MS=8e3,IPC_ABORT_TIMEOUT_HEALTH_MS=12e3,IPC_ABORT_TIMEOUT_MCP_MS=3e4,IPC_ABORT_TIMEOUT_RECALL_MS=2e4,IPC_ABORT_TIMEOUT_CORE_MS=15e3,
IPC_ABORT_TIMEOUT_SECONDARY_MS=2e4,IPC_ABORT_TIMEOUT_ANALYTICS_MS=6e4,IPC_TRANSPORT_MARGIN_MS=500
;function wait(ms){return new Promise(resolve=>setTimeout(resolve,ms))}function isAuthStatus(status){return status===401||status===403}
async function withTimeout(promise,timeoutMs,label){let timer=null;try{return await Promise.race([promise,new Promise((_,reject)=>{timer=setTimeout(()=>{
reject(new Error(`${label}: timed out after ${timeoutMs}ms`))},timeoutMs)})])}finally{timer&&clearTimeout(timer)}}function normalizePathForTimeoutRouting(path){
const raw=String(path||"").trim().toLowerCase();if(!raw)return"";if(raw.startsWith("http://")||raw.startsWith("https://"))try{const parsed=new URL(raw)
;return`${parsed.pathname||"/"}`.toLowerCase()+(parsed.search||"")}catch{return raw}return raw.startsWith("/")?raw:`/${raw}`}
function normalizeCortexBaseUrl(cortexBase){let url;try{url=new URL(String(cortexBase||"").trim())}catch{throw new Error("Cortex base URL must be a valid URL.")
...
;if(url.username||url.password)throw new Error("Cortex base URL must not include embedded credentials.");return url.hash="",url.search="",
url.toString().replace(/\/+$/,"")}function buildHttpFallbackUrl(cortexBase,path){
const normalizedPath=String(path||"").trim(),route=normalizedPath.startsWith("/")?normalizedPath:`/${normalizedPath}`
;return`${normalizeCortexBaseUrl(cortexBase)}${route}`}function formatHttpError(path,status,bodyText){if(!bodyText)return`${path}: HTTP ${status}`;try{
...
return!value||typeof value!="object"?!1:typeof value.status=="number"&&typeof value.body=="string"}function shouldFallbackToHttp(error){
const message=String(error?.message||error||"").toLowerCase()
;return message.includes("ipc request")||message.includes("task failed")||message.includes("invalid ipc response")||message.includes("read failed")||message.includes("write failed")||message.includes("os error 10060")||message.includes("connection attempt failed because the connected party did not properly respond")||message.includes("established connection failed because connected host has failed to respond")||message.includes("cannot connect to daemon")||message.includes("cannot set read timeout")||message.includes("cannot set write timeout")
}function errorMessage(error){return error instanceof Error?error.message:String(error??"")}function buildFallbackFailure(ipcError,httpError){
const ipcMessage=errorMessage(ipcError),httpMessage=errorMessage(httpError);return new Error(`${ipcMessage}; HTTP fallback failed: ${httpMessage}`)}
async function refreshTokenIfChanged(onTokenRefresh,getToken,previousToken){if(!onTokenRefresh)return!1;const requiresRotation=!!previousToken
;for(let attempt=1;attempt<=4;attempt+=1){await onTokenRefresh(previousToken,attempt);const nextToken=getToken()
;if(!!nextToken&&(!requiresRotation||nextToken!==previousToken))return!0;attempt<4&&await wait(250*attempt)}return!1}
function createApi({getInvoke:getInvoke,getToken:getToken,cortexBase:cortexBase,onTokenRefresh:onTokenRefresh}){
return async function api(path,withAuth=!1,_retried=!1){const invoke=getInvoke();let token=getToken();if(withAuth&&!token&&!_retried){
if(await refreshTokenIfChanged(onTokenRefresh,getToken,token))return api(path,withAuth,!0);token=getToken()}
if(withAuth&&!token)throw new Error(`${path}: no auth token (Tauri IPC ${invoke?"available":"missing"})`);const requestViaHttp=async()=>{const headers={
...
return async function postApi(path,body={},_retried=!1){const invoke=getInvoke();let token=getToken();if(!token&&!_retried){
if(await refreshTokenIfChanged(onTokenRefresh,getToken,token))return postApi(path,body,!0);token=getToken()}
...
const path=String(message||"").split(":")[0],normalized=Object.keys(PANEL_LABELS).find(candidate=>path.startsWith(candidate))
;return normalized?PANEL_LABELS[normalized]:null}function isAuthFailure(message){const text=String(message||"")
;return text.includes("HTTP 401")||text.includes("HTTP 403")||text.includes("no auth token")}function summarizeDashboardErrors(errors){
const unique=[...new Set((errors||[]).filter(Boolean))];if(!unique.length)return"";const authFailures=unique.filter(isAuthFailure)
