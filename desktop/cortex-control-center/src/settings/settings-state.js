const CONTROL_CENTER_SETTINGS_STORAGE_KEY = "cortex_control_center_settings",
  DEFAULT_CONTROL_CENTER_SETTINGS = Object.freeze({
    reducedMotion: "system",
    highContrast: !1,
    keyboardHints: !0,
    compactNavigation: !1,
  }),
  REDUCED_MOTION_VALUES = new Set(["system", "reduce", "full"]),
  BUDGET_ENDPOINT_DEFINITIONS = Object.freeze([
    {
      key: "store",
      label: "Store",
      defaultLimit: 120,
...
function resolveEffectiveReducedMotion(
  setting = "system",
  osPrefersReducedMotion = !1,
) {
...
function normalizeControlCenterSettings(value = {}) {
  const raw = value && typeof value == "object" ? value : {};
  return {
    reducedMotion: REDUCED_MOTION_VALUES.has(raw.reducedMotion)
      ? raw.reducedMotion
      : DEFAULT_CONTROL_CENTER_SETTINGS.reducedMotion,
...
function readControlCenterSettings(storage = globalThis?.localStorage) {
...
function writeControlCenterSettings(
  settings,
...
function readBool(value, fallback = !1) {
...
function readErrorMessage(value) {
  return value
    ? typeof value == "string"
      ? value
      : String(
          typeof value == "object" ? value.message || value.code || "" : value,
        )
    : "";
}
...
function summarizeBudgetStatus(budgets) {
  ...
  const configLoaded = readBool(
    ...
    enabled = readBool(budgets.enabled, !1),
    error = readErrorMessage(budgets.error),
    source = String(budgets.source || ""),
    endpoints =
      budgets.endpoints && typeof budgets.endpoints == "object"
        ? budgets.endpoints
        : {},
    recentDenials = readDenials(
      budgets.recentDenials ?? budgets.recent_denials,
    ),
    endpointRows = Object.entries(endpoints).map(([endpoint, config]) => {
...
function createBudgetDraftFromStatus(budgets) {
  const summary = summarizeBudgetStatus(budgets),
    byEndpoint = new Map(
      summary.endpointRows.map((row) => [row.endpoint, row]),
    ),
        ...
        return [
          ...
          row
            ? {
                ...
                windowSeconds:
                  row.windowSeconds ?? definition.defaultWindowSeconds,
              }
...
function normalizeBudgetDraft(value = {}) {
  const raw = value && typeof value == "object" ? value : {},
    rawEndpoints =
      ...
      BUDGET_ENDPOINT_DEFINITIONS.map((definition) => {
        ...
        return [
          definition.key,
          {
            ...
            windowSeconds:
              endpoint.windowSeconds ?? definition.defaultWindowSeconds,
          },
        ];
      }),
    );
...
function validateBudgetDraft(value) {
...
function serializeBudgetDraftForSave(value) {
  const draft = normalizeBudgetDraft(value);
  return {
    enabled: draft.enabled,
    endpoints: BUDGET_ENDPOINT_DEFINITIONS.map((definition) => {
      const endpoint = draft.endpoints[definition.key];
      return {
