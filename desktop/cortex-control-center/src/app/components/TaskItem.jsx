...
function TaskItem({ task, selectedOperator = "", completionDraft = "", completionExpanded = !1, onClaim = null, onAbandon = null, onComplete = null,
  onDelete = null, onCompletionDraftChange = null, onToggleComplete = null, busyActionKey = "", }) { const operator = String(selectedOperator || "").trim(),
    claimBusy = busyActionKey === `claim:${task.taskId}`, abandonBusy = busyActionKey === `abandon:${task.taskId}`,
    completeBusy = busyActionKey === `complete:${task.taskId}`, deleteBusy = busyActionKey === `delete:${task.taskId}`,
    operatorOwnsTask = canFinalizeTask(task, operator), files = Array.isArray(task.files) ? task.files.slice(0, 4) : [], detail = task.claimedBy
  ...
  return ( <li>
      ...
      {files.length ? ( <div className="feed-files">
          {files.map((file) => ( <span key={`${task.taskId}-${file}`} className="lock-path">
              {file}
            </span>
          ))}
        </div>
      ...
      <div className="task-actions">
        {canClaimTask(task, operator) && onClaim ? ( <button
            type="button"
        ...
        {task.status === "claimed" && operatorOwnsTask && onToggleComplete ? ( <button
            type="button"
            className="btn-sm"
        ...
        {task.status === "claimed" && operatorOwnsTask && onAbandon ? ( <button
            type="button"
        ...
        {task.status === "claimed" && !operatorOwnsTask && task.claimedBy ? ( <span className="surface-inline-hint">
            {"Held by "}
            {task.claimedBy}
          </span>
        ) : null}
        {task.status === "completed" && onDelete ? ( <button
            type="button"
            className="btn-sm"
      ...
      {completionExpanded && operatorOwnsTask && onComplete && onCompletionDraftChange ? ( <div className="task-complete-panel">
          <textarea
