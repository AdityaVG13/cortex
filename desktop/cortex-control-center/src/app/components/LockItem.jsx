...
function LockItem({ lock, selectedOperator = "", onUnlock = null, busyActionKey = "" }) {
  const expiryMinutes = Math.max(0, Math.ceil((new Date(lock.expiresAt).getTime() - Date.now()) / 6e4)), unlockBusy = busyActionKey === `unlock:${lock.path}`,
  ...
  return ( <li>
      ...
      {unlockable && onUnlock ? ( <div className="task-actions">
