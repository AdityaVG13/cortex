...
function buildLineGeometry(data, width, height, padding = 8) { if (!data || data.length < 2) return null;
  const numeric = data.map((value) => Number(value || 0)), max = Math.max(...numeric, 1), min = Math.min(...numeric, 0), range = max - min || 1,
    innerWidth = width - padding * 2, innerHeight = height - padding * 2,
    points = numeric.map((value, index) => { const x = padding + (index / (numeric.length - 1)) * innerWidth,
