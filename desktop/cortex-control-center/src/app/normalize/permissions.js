...
function normalizePermissionGrant(entry, index) { const client = String(pickDefined(entry?.client, entry?.client_id, entry?.clientId, "unknown") || "unknown"),
    permission = String(pickDefined(entry?.permission, "read") || "read").toLowerCase(), scope = String(pickDefined(entry?.scope, "*") || "*"),
    grantedBy = String(pickDefined(entry?.grantedBy, entry?.granted_by, "") || ""),
