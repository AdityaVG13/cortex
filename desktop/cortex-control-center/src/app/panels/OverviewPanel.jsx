...
function OverviewPanel() { const { panel, stats, tasks, locks, savings, conflictPairs,
  ...
  const setupTitle = invokeRef.current
      ...
      ["purple", "event", "Events", typeof stats.events == "number" ? stats.events : 0], ["green", "agents", "Active Agents", normalizedSessions.length],
    ], summaryCards = [ [ "30d median gain", formatMissionTokenValue(Number(monteCarloProjection?.summary?.p50Gain || 0), { signed: !0 }), monteCarloProjection
          ? `${monteCarloProjection.simulationCount} deterministic sims`
          : "Waiting for more history", ], [ "Boot Savings Run-Rate",
        formatMissionTokenValue(Number(monteCarloProjection?.summary?.avgDaily || 0), { perDay: !0 }), bootSavingsMomentum === null
          ? "Momentum pending"
          : `${bootSavingsMomentum >= 0 ? "+" : ""}${bootSavingsMomentum}% vs prior window`, ], [ "Work in flight",
        claimedTasks.length + pendingTasks.length, `${claimedTasks.length} claimed / ${pendingTasks.length} pending`,
      ], ["Knowledge Entries", memoryLoad, `${stats.memories} memories / ${stats.decisions} decisions`], ], memoryRows = [
      { label: "Headline recall hit rate", value: `${latestRecallHitRate || 0}%` }, { label: "7-day average", value: `${recallWindowAverage || 0}%` },
      { label: "Spread", value: `${recallWindowSpread || 0} pts` }, { label: "Conflict pairs", value: conflictPairs.length }, ];
  return ( <React.Fragment>
      {panel === "overview" ? ( <section className="panel active">
          <div className="panel-header overview-panel-header">
            ...
            <div className="surface-actions">
              ...
              <button type="button" className="btn-sm btn-primary" onClick={openEditorSetupWizard} disabled={!canSetupEditors} title={setupTitle}>
                {isSettingUpEditors ? "Setting Up..." : invokeRef.current ? "Setup MCP" : "Setup MCP (Desktop)"}
              </button>
            </div>
          </div>
          <div className="metrics overview-metrics">
            {metricCards.map(([accent, icon, label, value]) => ( <div key={label} className="metric" data-accent={accent}>
          ...
          {editorSetupSummary.results.length ? ( <div className="editor-setup-panel">
              ...
              <div className="editor-setup-grid">
                {editorSetupSummary.results.map((entry) => { const tone = entry.detected ? (entry.registered ? "ok" : "warn") : "idle",
                    stateLabel = entry.detected
                      ? entry.registered
                        ? "Configured"
                        : "Needs attention"
                      : "Not detected";
                  return ( <div key={entry.name} className={`editor-setup-item ${tone}`}>
          ...
          <div className="overview-dashboard-grid">
            <div className="card overview-hero-card overview-span-2">
              ...
              <div className="overview-unit-strip" role="group" aria-label="Mission Control unit key">
                ...
                {MISSION_METRIC_LEGEND.map((entry) => ( <span key={entry.abbreviation} className="overview-unit-chip">
              ...
              {showMissionMetricLegend ? ( <div
                  ...
                  <ul>
                    {MISSION_METRIC_LEGEND.map((entry) => ( <li key={entry.abbreviation}>
                        <code>{entry.abbreviation}</code>
                        {" = "}
                        {entry.meaning}
                      </li>
              ...
              <div className="overview-summary-grid">
                {summaryCards.map(([label, value, detail]) => ( <div key={label} className="overview-summary-card">
            ...
            <div className={`card onboarding-readiness-card ${firstRunReadiness.tone}`}>
              ...
              <div className="overview-status-list" aria-label="First-run readiness checklist">
                {firstRunReadiness.steps.map((step) => ( <div key={step.key} className="overview-status-row onboarding-step-row">
              ...
              <div className="overview-dual-stack">
                <div>
                  ...
                  <ul className="item-list compact-list">
                    {recentOverviewTasks.length ? ( recentOverviewTasks.map((task) => <TaskItem key={task.taskId} task={task} />)
