...
function useDashboardEffects(ctx) {
  const {
    panel,
    daemonState,
    healthMeta,
  ...
  (useEffect(() => {
    ...
    useEffect(() => {
      ...
      const budgetReloadTimer = window.setTimeout(
        () => {
          reloadBudgetConfigDraft({ silent: !0 });
        },
    ...
    useEffect(() => {
      ...
      const query = window.matchMedia("(prefers-reduced-motion: reduce)"),
        syncReducedMotion = () => setOsReducedMotion(!!query.matches);
      return (
        syncReducedMotion(),
    ...
    useEffect(() => {
      panel === "analytics" && setHasVisitedAnalytics(!0);
    }, [panel]),
    useEffect(() => {
      if (hasVisitedAnalytics) return;
      const warmupTimer = window.setTimeout(() => {
        startTransition(() => {
          (setHasVisitedAnalytics(!0), setAnalyticsReady(!0));
        });
      }, 250);
      return () => {
    ...
    useEffect(() => {
      checkForUpdates().then((update) => {
        update && setAvailableUpdate(update);
      });
    }, []),
    useEffect(() => {
    ...
    useEffect(() => {
      if (skipInitialFeedRefreshRef.current) {
        skipInitialFeedRefreshRef.current = !1;
        return;
      }
    ...
    useEffect(() => {
      if (skipInitialMessagesRefreshRef.current) {
        skipInitialMessagesRefreshRef.current = !1;
        return;
      }
    ...
    useEffect(() => {
      if (skipInitialActivityRefreshRef.current) {
        skipInitialActivityRefreshRef.current = !1;
        return;
      }
  ...
  const pendingTasks = useMemo(
    ...
    sidebarUtilityStats = useMemo(
      () => [
        {
          label: "Queue",
          value: pendingTasks.length,
          tone: pendingTasks.length ? "warning" : "calm",
        },
        {
          label: "Locks",
          value: locks.length,
          tone: locks.length ? "cyan" : "calm",
        },
        {
          label: "Recall",
          value: `${latestRecallHitRate || 0}%`,
          tone: latestRecallHitRate >= 85 ? "green" : "warning",
        },
        {
          label: "Agents",
    ...
    reportSurfaceError = useCallback((error) => {
      const message = error?.message || String(error);
      !message ||
        isDaemonSuppressibleErrorMessage(message) ||
        setFeedbackMessage(summarizeDashboardErrors([message]) || message);
    }, []),
    handleTaskClaim = useCallback(
      async (task) => {
    ...
    handleTaskComplete = useCallback(
      async (task, summary) => {
        const operator = selectedOperatorName;
        if (!operator) {
