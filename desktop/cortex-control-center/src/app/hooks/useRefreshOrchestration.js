...
function useRefreshOrchestration(ctx) {
  const {
      panel,
      stats,
      sessions,
    ...
    callMcpTool = useCallback(
      async (name, args = {}) => {
        const payload = await postApi("/mcp-rpc", {
          ...
          error = extractMcpToolError(payload);
        if (error) throw new Error(`MCP ${name} failed: ${error}`);
        return parseMcpToolResult(payload?.result) ?? payload?.result ?? null;
      },
      [postApi],
    ),
    writeDevVerificationReport = useCallback(
      async (report) =>
        DEV_RESTART_VERIFY_ENABLED
          ? call("write_dev_verification_report", {
    ...
    refreshDaemonState = useCallback(async () => {
      if (invokeRef.current)
        try {
    ...
    probeReadiness = useCallback(async () => {
      try {
    ...
    refreshHealth = useCallback(async () => {
      ...
      if (
        (setHealthMeta({
          ...
          dbCorrupted: !!health?.db_corrupted,
          runtimeVersion,
    ...
    refreshCoreData = useCallback(
      async (options = {}) => {
        const throwOnError = options?.throwOnError !== !1,
          jobs = [
            {
              fn: () => api("/sessions", !0),
              apply: (v) =>
        ...
        (results.forEach((result, index) => {
          if (result.status === "fulfilled") {
            (jobs[index].apply(result.value), (successCount += 1));
            return;
          }
    ...
    refreshSavings = useCallback(async () => {
      const result = await api("/savings", !0);
      (result && setSavings(result), clearTransientFeedback());
    }, [api, clearTransientFeedback]),
    refreshConflicts = useCallback(async () => {
      const result = await api("/conflicts", !0),
        normalizedPairs = normalizeConflictPairsPayload(result);
        ...
        if (!(
          ...
          try {
            ...
            if (String(error?.message || error || "").includes("HTTP 403")) {
              ((permissionsEndpointAvailableRef.current = !0),
                setPermissionsEndpointAvailable(!0),
                setPermissionAccessDenied(!0),
                setPermissionGrants([]));
              return;
            }
            if (isRouteMissingError(error)) {
              ((permissionsEndpointAvailableRef.current = !1),
                setPermissionsEndpointAvailable(!1),
                setPermissionAccessDenied(!1),
                setPermissionGrants([]),
                clearTransientFeedback());
              return;
    ...
    refreshSecondaryData = useCallback(
      async (options = {}) => {
        const force = options?.force === !0,
          wantsWorkStreams = panel === "work" || panel === "overview",
    ...
    refreshProtectedData = useCallback(
      async (options = {}) => {
        const includeSecondary = options?.includeSecondary !== !1,
        ...
        if (shouldRefreshCore) {
          const coreRefresh = await refreshCoreData({ throwOnError: !1 });
          ((coreErrors = coreRefresh.errors),
            (coreSuccessCount = coreRefresh.successCount),
            (coreTotalCount = coreRefresh.totalCount),
            coreErrors.length || (lastCoreRefreshAtRef.current = Date.now()));
        }
      ...
      [refreshCoreData, refreshSecondaryData],
    ),
    refreshSecondaryDataInBackground = useCallback(() => {
      typeof window > "u" ||
        startupSecondaryRefreshInFlightRef.current ||
        ((startupSecondaryRefreshInFlightRef.current = !0),
        window.setTimeout(() => {
          (async () => {
            if (!daemonStateRef.current?.reachable) return;
            ...
            startupSecondaryRefreshInFlightRef.current = !1;
          });
        }, 0));
    }, [refreshSecondaryData, setSecondaryAvailabilityFeedback]),
    refreshProtectedDataForStartup = useCallback(async () => {
      const includeSecondary = startupCoreReadyRef.current;
      let result = await refreshProtectedData({
        includeSecondary,
        forceCore: !0,
      });
      return (
        !includeSecondary &&
          !result.coreErrors.length &&
    ...
    clearStartupCoreReady = useCallback(() => {
      ((startupCoreReadyRef.current = !1),
        setStartupCoreReadyState(!1),
      ...
      async (keepId, action, supersededId, pair = null) => {
        ...
        try {
          try {
            ...
            if (!isRouteMissingError(primaryError)) throw primaryError;
          ...
          setConflictLoading(!1);
        }
      },
      [postApi, refreshConflicts, selectedOperatorName],
    ),
    handleResolveDraftChange = useCallback((pairKey, updates) => {
      setResolveDrafts((current) => {
    ...
    updateBudgetDraftRoot = useCallback((patch) => {
    ...
    updateBudgetEndpointDraft = useCallback((endpoint, patch) => {
      (setBudgetDraftDirty(!0),
        setBudgetConfigMessage(""),
        setBudgetDraft((current) => {
          ...
          return {
            ...
            endpoints: {
              ...base.endpoints,
              [endpoint]: { ...base.endpoints[endpoint], ...patch },
            },
          };
        }));
    }, []),
    ...
    saveBudgetConfigDraft = useCallback(
      async (event) => {
        if ((event.preventDefault(), !invokeRef.current)) {
          setBudgetConfigMessage("Budget editing requires the desktop app.");
          return;
        }
