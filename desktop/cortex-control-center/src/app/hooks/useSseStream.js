...
function useSseStream(ctx) { const { daemonState, cortexBase, refreshAllRef, tokenRef, streamConnectedAtRef, streamSessionEventCountRef,
  ...
  return ( useEffect(() => { let stream = null, refreshTimer = null, reconnectTimer = null, reconnectAttempt = 0, lastRefreshAt = 0, refreshInFlight = !1,
        refreshQueued = !1, disposed = !1;
      const clearRefreshTimer = () => { refreshTimer && (window.clearTimeout(refreshTimer), (refreshTimer = null));
        }, clearReconnectTimer = () => { reconnectTimer && (window.clearTimeout(reconnectTimer), (reconnectTimer = null)); },
        scheduleRefresh = (immediate = !1) => { if (disposed || refreshTimer) return;
          const elapsed = Date.now() - lastRefreshAt, delay = immediate ? 0 : Math.max(SSE_REFRESH_THROTTLE_MS - elapsed, 0);
          refreshTimer = window.setTimeout(() => { if (((refreshTimer = null), refreshInFlight)) { refreshQueued = !0;
              return;
            }
            ((refreshInFlight = !0), Promise.resolve(refreshAllRef.current()).finally(() => { ((lastRefreshAt = Date.now()), (refreshInFlight = !1),
          ...
          ((stream = nextStream), (nextStream.onopen = () => { ((reconnectAttempt = 0), (streamConnectedAtRef.current = Date.now()), scheduleRefresh(!0));
            ...
            nextStream.addEventListener("connected", handleRealtimeEvent), nextStream.addEventListener("task", handleRealtimeEvent),
            nextStream.addEventListener("session", () => { ((streamSessionEventCountRef.current += 1), handleRealtimeEvent());
            }), nextStream.addEventListener("lock", handleRealtimeEvent),
            nextStream.addEventListener("feed", handleRealtimeEvent), nextStream.addEventListener("message", handleRealtimeEvent),
            nextStream.addEventListener("activity", handleRealtimeEvent), (nextStream.onerror = () => { disposed || stream !== nextStream ||
                ((streamDisconnectedAtRef.current = Date.now()), handleRealtimeEvent(), closeStream(), scheduleReconnect());
            })); }, handleOnline = () => { disposed || ((reconnectAttempt = 0), clearReconnectTimer(), closeStream(), connect(), scheduleRefresh(!0));
        };
