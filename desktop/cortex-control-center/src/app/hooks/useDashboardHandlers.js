...
function useDashboardHandlers(ctx) {
  ...
  async function handleStartDaemon() {
    if (invokeRef.current) {
      (resetStartupRetryState(), (daemonTransitionRef.current = !0));
      try {
  ...
  (useEffect(() => {
      ...
      (async () => {
        ...
        try {
          ...
          if (
            ...
            recordStep("start", {
              message: "Daemon already reachable before verification.",
            });
          else {
            const startResult = await call("start_daemon");
            if (
              (recordStep("start", {
                message: startResult?.message || "Daemon start requested.",
              }),
          ...
          ((report.completedAt = new Date().toISOString()),
            (report.finalDaemonState = {
              running: !!daemonStateRef.current?.running,
              reachable: !!daemonStateRef.current?.reachable,
              managed: !!daemonStateRef.current?.managed,
              authTokenReady: !!daemonStateRef.current?.authTokenReady,
          ...
          if (((completed = !0), await sleep(500), invokeRef.current))
            try {
              await call("quit_app");
            } catch {}
        }
    ...
    useEffect(() => {
      if (!showEditorSetupWizard) return;
      const frame = window.requestAnimationFrame(() => {
  ...
  function handleFirstRunAction() {
    if (!firstRunReadiness.action.disabled)
  ...
  const activePanelLabel = PANEL_SEQUENCE_LABEL.get(panel) || "Overview",
    ...
    hostLabel = connectionEndpoint.hostLabel,
    handleAnalyticsTabKey = useCallback(
      (event) => {
        const order = ["aggregate", "operations"],
          currentIndex = Math.max(0, order.indexOf(analyticsMode));
        let nextIndex = null;
        if (
          (event.key === "ArrowRight" || event.key === "ArrowDown"
            ...
            : event.key === "ArrowLeft" || event.key === "ArrowUp"
              ...
              : event.key === "Home"
                ? (nextIndex = 0)
                : event.key === "End" && (nextIndex = order.length - 1),
          nextIndex === null)
        )
          return;
