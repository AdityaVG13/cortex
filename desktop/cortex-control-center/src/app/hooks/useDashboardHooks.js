...
function useDashboardHooks() { let ctx = useDashboardState();
  return ( (ctx = useRefreshOrchestration(ctx)), (ctx = useRefreshAll(ctx)), (ctx = useDashboardEffects(ctx)),
    (ctx = useSseStream(ctx)), (ctx = useDaemonConnection(ctx)), useDashboardHandlers(ctx) );
}
