  ...
  const safeWindow = Math.max(1, Math.floor(Number(windowDays) || 1)),
    now =
      nowDate instanceof Date && !Number.isNaN(nowDate.getTime())
        ? nowDate
        : new Date(),
    endDay = parseIsoUtcDay(toIsoUtcDay(now));
  return endDay
    ? Array.from({ length: safeWindow }, (_, index) => {
        const day = new Date(
          endDay.getTime() - (safeWindow - 1 - index) * 864e5,
        );
...
function summarizeBootThroughput(
  dailySeries,
  windowDays = 7,
  nowDate = new Date(),
) {
  const safeWindow = Math.max(1, Math.floor(Number(windowDays) || 7)),
    byDay = normalizeBootRowsByDay(dailySeries),
    windowDaysIso = trailingIsoDays(safeWindow, nowDate),
    windowStart = windowDaysIso[0] || "",
    windowEnd = windowDaysIso.at(-1) || "",
    boots = windowDaysIso.reduce(
      (sum, day) => sum + Number(byDay.get(day) || 0),
      0,
    ),
    firstObservedDay = [...byDay.keys()].sort()[0] || "";
  let daysRepresented = safeWindow;
  firstObservedDay
    ? windowStart &&
      firstObservedDay > windowStart &&
      (daysRepresented = Math.min(
        safeWindow,
        daysBetweenInclusive(firstObservedDay, windowEnd),
      ))
    : (daysRepresented = 0);
  const avgPerDay =
    daysRepresented > 0 ? Math.round((boots / daysRepresented) * 10) / 10 : 0;
  return {
