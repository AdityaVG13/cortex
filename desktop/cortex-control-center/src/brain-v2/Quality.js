const BRAIN_QUALITY_BUDGETS = Object.freeze({
...
function detectBrainQualityTier({
  ...
  const width = Number(windowObj?.innerWidth) || 1024,
    ...
    cores = Number(navigatorObj?.hardwareConcurrency) || 4,
    memory = Number(navigatorObj?.deviceMemory) || 4,
    userAgent = String(navigatorObj?.userAgent || ""),
    mobileUserAgent = /Android|iPhone|iPad|iPod/i.test(userAgent),
    hasFinePointer =
    ...
    low =
      ...
      cores <= 2 ||
      memory <= 2,
    medium = !low && (width < 1100 || cores <= 4 || memory <= 4),
    tier = low ? "low" : medium ? "medium" : "high";
  return {
    tier,
    nodeBudget: brainBudgetForQuality(tier),
    pixelRatio: pixelRatioForTier(tier, dpr),
    idleFiring: tier !== "low",
    hitRadiusScale: tier === "low" ? 3 : 2,
  };
}
