...
const AUTO_ROTATE_RATE = 0.04,
  AUTO_RESUME_MS = 8e3,
  SPOTLIGHT_DURATION_MS = 1200,
  RETURN_DURATION_MS = 900;
function createCamera({ camera, controls, autoRotate = !0 }) {
  ...
  function tick(now = performance.now()) {
    ...
    if (autoRotate && idle >= AUTO_RESUME_MS) {
      const angle = AUTO_ROTATE_RATE * dt,
        ...
        cx = camera.position.x - tx,
        cz = camera.position.z - tz;
