...
function createBeams({ scene }) {
  const totalVerts = POOL_SIZE * VERTS_PER_BEAM,
    ...
    progresses = new Float32Array(totalVerts),
    beamIds = new Float32Array(totalVerts),
    colors = new Float32Array(totalVerts * 3),
    indices = new Uint16Array(POOL_SIZE * SEGMENTS * 2);
  ...
  function findSlot(now) {
    for (let i = 0; i < POOL_SIZE; i += 1) if (!slots[i].active) return i;
    let oldestIdx = 0,
      oldestTime = 1 / 0;
    for (let i = 0; i < POOL_SIZE; i += 1)
      slots[i].startTime < oldestTime &&
        ((oldestTime = slots[i].startTime), (oldestIdx = i));
    return oldestIdx;
  }
  function fire({
    from,
    ...
    return (
      ...
      (geometry.attributes.aColor.needsUpdate = !0),
      (slots[slot].active = !0),
  ...
  function tick(now = performance.now()) {
    ...
    for (let i = 0; i < POOL_SIZE; i += 1) {
      const slot = slots[i];
      if (!slot.active) {
        data[i] !== 0 && ((data[i] = 0), (dirty = !0));
        continue;
      }
      const t = now - slot.startTime;
      if (t >= slot.lifeMs) {
        ((slot.active = !1), (data[i] = 0), (dirty = !0));
        continue;
      }
      const value = riseDecay(t, RISE_MS, TAU_MS);
      ((data[i] = Math.min(1, value)), (dirty = !0));
    }
    (dirty && (texture.needsUpdate = !0),
      (material.uniforms.uTime.value = (now * 0.001) % 1e3));
  }
