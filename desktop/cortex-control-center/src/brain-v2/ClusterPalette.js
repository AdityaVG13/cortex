...
const GOLDEN_ANGLE = 137.508,
  ...
  LIGHTNESS = 0.58,
  cache = new Map(),
  DECISION_COLOR = new THREE.Color("#ffd166"),
  LOOSE_COLOR = new THREE.Color("#22d3ee"),
  SELECTED_COLOR = new THREE.Color("#ffffff");
function paletteForCluster(centroidBytes) {
  const seed = fnv1a32(centroidBytes),
    cached = cache.get(seed);
  if (cached) return cached;
  const hue = (((seed >>> 0) * GOLDEN_ANGLE) / 4294967296) % 360,
