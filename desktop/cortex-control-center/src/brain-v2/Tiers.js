...
function clusterLabelOf(cluster) {
  const label = cluster?.label,
    consolidated = cluster?.consolidated_text;
...
function decisionLabelOf(dec) {
  ...
  return decision && decision.trim().length > 0
    ? truncate(decision)
    : `Decision ${dec?.id ?? "?"}`;
}
const TIER_DECISION_RADIUS = 80,
  TIER_CLUSTER_RADIUS = 140,
  TIER_LOOSE_RADIUS_MIN = 180,
  TIER_LOOSE_RADIUS_MAX = 220,
  TOTAL_BUDGET_MIN = 70,
  TOTAL_BUDGET_MAX = 90,
  DECISION_RATIO = 0.15,
  CLUSTER_RATIO = 0.55,
...
function pickBudget() {
  const span = TOTAL_BUDGET_MAX - TOTAL_BUDGET_MIN + 1,
    total = TOTAL_BUDGET_MIN + Math.floor(Math.random() * span),
    decisions = Math.max(3, Math.round(total * DECISION_RATIO)),
    clusters = Math.max(10, Math.round(total * CLUSTER_RATIO)),
    looseTarget = Math.max(5, Math.round(total * LOOSE_RATIO)),
    roundedTotal = decisions + clusters + looseTarget,
    loose = Math.max(5, looseTarget + total - roundedTotal);
  return { total, decisions, clusters, loose };
}
function fibonacciOnSphere(index, total, seedOffset = 0) {
  const i = index + 0.5,
    phi = Math.acos(1 - (2 * i) / Math.max(total, 1)),
    theta = GOLDEN_ANGLE * i + seedOffset;
  return {
...
function clusterRadius(memberCount) {
  return !memberCount || memberCount < 1
    ? 1.4
    : Math.min(4, Math.max(1.4, Math.log2(memberCount + 1) * 1.4));
}
function looseRadius(seed) {
  const f = ((seed >>> 0) % 1024) / 1024;
  return (
    TIER_LOOSE_RADIUS_MIN + (TIER_LOOSE_RADIUS_MAX - TIER_LOOSE_RADIUS_MIN) * f
  );
}
function buildTiers(dump, options = {}) {
  const budget = options.budget || pickBudget(),
    decisions = (dump?.decisions || []).slice(0, budget.decisions),
    rawClusters = (dump?.clusters || dump?.crystals || []).slice(
      0,
      budget.clusters,
    ),
  ...
  const desiredTotal = budget.decisions + budget.clusters + budget.loose,
    usedSoFar = decisions.length + rawClusters.length,
    looseTargetEarly = Math.max(budget.loose, desiredTotal - usedSoFar),
    looseMemories = memories.slice(0, looseTargetEarly),
    decisionsLayout = decisions.map((node, index) => {
      const id = `decision-${node.id}`,
        seed = fnv1a32(id),
        { nx, ny, nz } = fibonacciOnSphere(
          index,
          decisions.length,
          (seed % 1024) / 1024,
        );
    ...
    useColdStart = clusterCount === 0 && looseMemories.length > 0,
    clusterSourceCount = useColdStart
      ? Math.min(budget.clusters, looseMemories.length)
      : clusterCount,
    clustersLayout = useColdStart
      ...
      : rawClusters.map((cluster, index) => {
          const id = `cluster-${cluster.id}`,
            ...
            memberCount = Number(cluster.member_count || 1),
            bodyRadius = clusterRadius(memberCount);
          return {
            id,
            sourceId: cluster.id,
            tier: "cluster",
    ...
    usedClusters = clustersLayout.length,
    looseTarget = Math.max(
      budget.loose,
      desiredTotal - usedDecisions - usedClusters,
    ),
    loosePool = useColdStart
      ? looseMemories.slice(clusterSourceCount)
      : looseMemories,
    looseLayout = loosePool.slice(0, looseTarget).map((mem, index) => {
