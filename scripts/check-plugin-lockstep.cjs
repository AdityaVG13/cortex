...
if(!fs.existsSync(cargoTomlPath)){die(`Cargo.toml not found: ${cargoTomlPath}`)}const text=fs.readFileSync(cargoTomlPath,"utf8")
;const pkgSection=text.split(/^\[/m)[1];if(!pkgSection||!pkgSection.startsWith("package]")){die("Cargo.toml does not appear to open with a [package] section")}
const sectionBody=pkgSection.slice("package]".length);const match=sectionBody.match(/^\s*version\s*=\s*"([^"]+)"\s*$/m);if(!match){
die(`Could not find version in [package] of ${cargoTomlPath}`)}return match[1]}function readPluginVersion(){if(!fs.existsSync(pluginJsonPath)){
die(`plugin.json not found: ${pluginJsonPath}`)}let obj;try{obj=JSON.parse(fs.readFileSync(pluginJsonPath,"utf8"))}catch(e){
...
warn(`prepare-runtime.cjs hard-coded fallback version ${fallback} ≠ ${cargoVersion}. `+'Update the `let version = "..."` line when bumping.')}
console.error("[lockstep] PASS — daemon and plugin versions are lockstep.");process.exit(0)}main();
