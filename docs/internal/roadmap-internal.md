...
| ✓115 | Desktop app: fix About icon, update License field, version bump to v0.4.0 | D4 | Done - icon uses BASE_URL, license MIT (relicensed from AGPL-3.0 post-tag in df3138a), version 0.4.0 |
| ✓116 | Version bump ALL files to v0.4.0 + license sweep | D4 | Done - all Cargo.toml, package.json, README updated; 49 .rs files have SPDX headers; relicensed to MIT in df3138a |
...
| R1 | CrossEncoder reranking on recall | SmartSearch | CC | Rerank top-K results with cross-encoder; SmartSearch shows ranking > structure for retrieval quality |
| R2 | Score-adaptive truncation for boot | SmartSearch | CC | Dynamically preserve high-quality evidence within token budgets instead of fixed capsule sizes |
| R3 | Multi-graph memory views | MAGMA | CC | Orthogonal semantic/temporal/causal/entity views on memories; policy-guided traversal for recall |
| R4 | Zettelkasten auto-linking on store | A-MEM | D5 | When storing, auto-link to related existing memories via semantic similarity + shared tags |
| R5 | Decision quality gating (evaluator pass) | Anthropic Harness | CC | Independent evaluator judges store quality before persisting; separate generation from assessment |

### Research References (v0.5.0)

Papers and articles informing the v0.5.0 roadmap:

| Paper | Key Finding | Cortex Relevance |
|-------|------------|-----------------|
| [A-MEM: Agentic Memory for LLM Agents](https://arxiv.org/abs/2502.12110) (NeurIPS 2025) | Zettelkasten-inspired auto-linking creates interconnected knowledge networks; dynamic memory evolution where new memories update existing ones | R4: auto-linking on store |
| [MAGMA: Multi-Graph Agentic Memory Architecture](https://arxiv.org/abs/2601.03236) (2026) | Orthogonal semantic/temporal/causal/entity graphs with policy-guided traversal outperform unified memory stores | R3: multi-graph views |
| [SmartSearch: How Ranking Beats Structure](https://arxiv.org/abs/2603.15599) (Derehag et al., 2026) | Ranking > structure for retrieval; 98.6% recall but only 22.5% survives truncation without smart ranking; 8.5x fewer tokens | R1, R2: reranking + adaptive truncation |
| [Harness Design for Long-Running Apps](https://www.anthropic.com/engineering/harness-design-long-running-apps) (Anthropic, 2025) | Generator-evaluator separation beats self-evaluation; context resets > compaction; criteria-based quality gating | R5: decision quality gating |
