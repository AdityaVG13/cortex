...
pub use super::promote_policy::{
    PromotionRule, ResolvedPolicy, SCHEMA as POLICY_SCHEMA, ensure as ensure_policy,
    load as load_policy, resolve, set as set_policy,
};
...
pub fn promote(
    ...
    let policy = promote_policy::resolve(conn)?;
    if !policy.allows_principal(&promotion.rule, promotion.principal) {
        return Err(promote_policy::principal_denied(
            &promotion.rule,
            promotion.principal,
            &policy,
        ));
    }
    let revocation = policy.rule(&promotion.rule).revocation.clone();
    let (kind, population, exclusions, authority): (&str, Vec<String>, Vec<String>, String) =
        match promotion.rule {
            ...
            PromotionRule::Procedure => {
                ...
                if successes.len() < 2 || origins.len() < 2 || distinct_agents.len() < 2 {
                    return Err(format!(
                        "a procedure needs at least two independent successful cases with matching preconditions; found {} cases, {} independent origins, {} agents",
                        successes.len(),
                        origins.len(),
                        distinct_agents.len()
                    ));
    ...
    let count = crate::db::count_sql(
        conn,
        "SELECT COUNT(*) FROM records WHERE kind = ?1",
        params![kind],
    )?;
    let record_id = format!("{kind}:{}", count + 1);
    let body = json!({"text":promotion.text,"rule":promotion.rule.as_str(),"rule_version":PROMOTION_RULES_VERSION,"authority":authority,"population":population,"exclusions":exclusions,"preconditions":promotion.preconditions,"scope":promotion.target_scope.unwrap_or(DEFAULT_SCOPE),"revocation_triggers":revocation,"promoted_by":promotion.agent});
