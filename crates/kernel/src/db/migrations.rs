    ...
    match version {
        ...
        "019" => {
            ...
            if table_exists(conn, "traces")
                && table_exists(conn, "versions")
                && table_has_column(conn, "decisions", "version_id")
            {
                Ok(())
            } else {
                Err(migration_error(
        ...
        "023_authoritative_records" => {
            ...
            ensure_column(
                conn,
                "memories",
                "ALTER TABLE memories ADD COLUMN trust_basis TEXT DEFAULT 'legacy_model_weight'",
            )?;
            ensure_column(
                conn,
                "decisions",
                "ALTER TABLE decisions ADD COLUMN trust_basis TEXT DEFAULT 'legacy_model_weight'",
            )?;
            let imported = super::records::import_legacy(conn)?;
        ...
        "024_boot_capsule_indexes" => {
            ...
            conn.execute_batch(
                "CREATE INDEX IF NOT EXISTS idx_decisions_updated_at ON decisions(updated_at);
                 CREATE INDEX IF NOT EXISTS idx_decisions_status_retention_created ON decisions(status, retention_class, created_at, id);",
            )?;
            Ok(())
        }
