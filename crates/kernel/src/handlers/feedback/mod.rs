...
pub use recall::{
    compute_boosts, has_retrieval_immunity, parse_source, IMMUNITY_THRESHOLD, IMMUNITY_WINDOW_DAYS,
};
...
pub fn record_agent_feedback_from_value(
    ...
    let outcome = normalize_outcome(value_str(args, &["outcome"])).ok_or_else(|| {
    ...
    let outcome_score = value_f64(args, &["outcome_score", "outcomeScore"])
        ...
        .clamp(0.0, 1.0);
    let quality_score = value_f64(args, &["quality_score", "qualityScore"])
        .unwrap_or(0.7)
        .clamp(0.0, 1.0);
...
pub fn build_agent_feedback_stats_payload(
    ...
    Ok(
        json!({"ownerId":owner_id,"horizonDays":horizon_days,"limit":limit,"sampled":overall.count,"reliability":reliability,
...
pub fn recommend_recall_k(
    ...
    Ok(Some(
        json!({"agent":agent,"taskClass":task_class,"samples":count,"baseK":base_k,"recommendedK":recommended_k,"reason":reason,
