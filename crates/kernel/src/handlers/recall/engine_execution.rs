pub async fn emit_recall_query_event(cx: &asupersync::Cx, state: &RuntimeState, agent: &str, source_prefix: Option<&str>, payload: Value) -> Result<(), String> {
    ...
    let deferred = crate::state::DeferredSideEffect::Event { event_type: "recall_query".into(), payload: payload.clone(), agent: agent.to_string() };
    let agent = agent.to_string();
    state
        .with_write_or_defer(cx,
            move |conn| {
...
pub async fn bump_retrievals_bounded(cx: &asupersync::Cx, state: &RuntimeState, results: &[RecallItem]) -> Result<(), String> {
    let sources: Vec<String> = results.iter().map(|r| r.source.clone()).collect();
    if sources.is_empty() {
        return Ok(());
    }
    let deferred = crate::state::DeferredSideEffect::Retrievals { sources: sources.clone() };
    state.with_write_or_defer(cx, move |conn| bump_retrievals_sources(conn, &sources), deferred).await.map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn prefer_family_candidate(candidate: &RecallItem, current: &RecallItem, alignment_profile: &QueryAlignmentProfile) -> bool {
...
pub fn compact_budget_family_candidates(candidates: Vec<RecallItem>, query_text: &str, token_budget: usize) -> Vec<RecallItem> {
    compact_budget_family_candidates_with_trace(candidates, query_text, token_budget).0
}
pub fn apply_semantic_budget(raw: Vec<RecallItem>, token_budget: usize, query_text: &str) -> Vec<RecallItem> {
    if token_budget == 0 {
        return raw
            .into_iter()
            .map(|mut item| {
...
pub async fn execute_unified_recall(
    ...
    let (results, semantic_route, fail_closed, route_trace) = {
        ...
        let route_trace = take_last_route_trace().map(|t| serde_json::to_value(t).unwrap_or(Value::Null)).unwrap_or(Value::Null);
    ...
    if let Some(id_str) = source.strip_prefix("decision::") {
        if let Ok(id) = id_str.parse::<i64>() {
            if let Some((decision, context, owner_id, visibility)) = query_decision_by_id_for_unfold(conn, id) {
                if is_visible(owner_id, visibility.as_deref(), ctx) {
                    // Cold placement: the exact bytes live in the cold
                    // segment; a route marker is never served as the source.
                    if crate::db::cold::is_cold_marker(&decision) {
                        return match crate::db::cold::hydrate(conn, "decision", id) {
                            Ok(Some((text, cold_context, intact))) => {
                                let full = match cold_context {
                                    Some(c) => format!("{text}\n\nContext: {c}"),
                                    None => text,
                                };
                                Some(json!({"text":full,"type":"decision","physical_state":"cold_segment","intact":intact}))
                            }
                            _ => Some(json!({"text":Value::Null,"type":"decision","physical_state":"unavailable","error":"cold block unreadable"})),
                        };
                    }
                    ...
                    return Some(json!({"text":full,"type":"decision","physical_state":"inline"}));
                }
            }
        }
    }
...
pub fn query_decision_by_context_for_unfold(conn: &Connection, source: &str) -> Option<DecisionUnfoldRow> {
