...
pub fn recall_to_json(item: RecallItem) -> Value {
    ...
    if let Value::Object(ref mut map) = payload {
        ...
        if let Some(valid_from) = item.valid_from.as_deref() {
            map.insert("validFrom".to_string(), Value::String(valid_from.to_string()));
        }
        if let Some(valid_until) = item.valid_until.as_deref() {
            map.insert("validUntil".to_string(), Value::String(valid_until.to_string()));
        }
...
pub fn enforce_budget_token_invariant(results: Vec<RecallItem>, token_budget: usize, query_text: &str) -> Vec<RecallItem> {
...
pub fn collapse_score_is_better(candidate_score: f64, candidate_order: usize, best_score: f64, best_order: usize) -> bool {
...
pub async fn load_collapsed_source_fallback(cx: &asupersync::Cx, state: &RuntimeState, source: &str, query: &str, ctx: &RecallContext, relevance: f64) -> Result<Option<RecallItem>, String> {
    let conn = state.db_read.lock(cx).await.map_err(|e| e.to_string())?;
    let Some(payload) = unfold_source(&conn, source, ctx) else { return Ok(None); };
    let canonical_source = payload.get("source").and_then(|value| value.as_str()).unwrap_or(source).to_string();
    let Some(text) = payload.get("text").and_then(|value| value.as_str()) else { return Ok(None); };
    Ok(Some(RecallItem {
...
pub async fn dedup_and_mark_served(cx: &asupersync::Cx, state: &RuntimeState, agent: &str, query: &str, ctx: &RecallContext, results: Vec<RecallItem>) -> Result<Vec<RecallItem>, String> {
    if results.is_empty() {
        return Ok(results);
    }
    ...
    let mut seen_hashes: HashSet<u32> = {
        let mut served = state.served_content.lock(cx).await.map_err(|e| e.to_string())?;
        ...
        if already_served {
            if result.method == "crystal" && !result.collapsed_sources.is_empty() {
                ...
                for (order, collapsed_source, collapsed_score) in fallback_candidates {
                    ...
                    let Some(candidate) = load_collapsed_source_fallback(cx, state, &collapsed_source, query, ctx, candidate_relevance).await? else {
                        continue;
                    };
...
#[allow(dead_code)]
pub fn maybe_apply_rerank(_state: &RuntimeState, results: Vec<RecallItem>, budget: usize) -> (Vec<RecallItem>, Value) {
    let reason = if budget == 0 { "headlines_mode" } else { "model_free" };
    (results, json!({"status":"skipped","reason":reason,"mode":"off"}))
}
