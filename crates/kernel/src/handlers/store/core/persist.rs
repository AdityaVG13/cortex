...
pub fn store_decision_internal(
    ...
    if decision_truncated {
        let _ = log_event(
            conn,
            "decision_truncated",
            json!({"source_agent":source_agent,"entry_type":entry_type.as_str(),"original_chars":decision_chars,"stored_chars":MAX_DECISION_CHARS,"preview":truncate_chars(decision,180),}),
            "rust-daemon",
        );
    }
...
pub fn store_decision_legacy(
    ...
    let candidate_ids: Vec<i64> = recent_candidates
        .iter()
    ...
    if !is_typed_evidence_kind(entry_type) && surprise < 0.25 {
        let _ = log_event(
            ...
            json!({"decision":truncate_chars(decision, 100),"surprise":surprise,"source_agent":source_agent,"quality":quality,}),
            "rust-daemon",
        );
        checkpoint_wal_best_effort(conn);
        let mut entry =
            json!({"stored":false,"reason":"duplicate","surprise":surprise,"quality":quality,});
