...
pub type ReadConnLockFuture<'a> =
    Pin<Box<dyn Future<Output = Result<MutexGuard<'a, Connection>, LockError>> + Send + 'a>>;
pub trait ReadConnectionProvider: Send + Sync {
    fn lock<'a>(&'a self, cx: &'a Cx) -> ReadConnLockFuture<'a>;
...
impl ReadConnectionProvider for Mutex<Connection> {
    fn lock<'a>(&'a self, cx: &'a Cx) -> ReadConnLockFuture<'a> {
        Box::pin(async move { Mutex::lock(self, cx).await })
    }
}
...
impl ReadConnectionProvider for ReadConnectionPool {
    fn lock<'a>(&'a self, cx: &'a Cx) -> ReadConnLockFuture<'a> {
        ...
        Box::pin(async move { self.connections[idx].lock(cx).await })
    }
    fn pool_size(&self) -> usize {
        self.connections.len()
    }
}
pub fn derive_read_pool_size(configured: Option<usize>, cpu_hint: Option<usize>) -> usize {
...
pub fn read_pool_size_from_env() -> usize {
...
pub fn open_query_only_connection(db_path: &Path) -> Result<Connection, String> {
    let read_conn =
