...
pub fn migrate_history_tables(conn: &Connection) -> rusqlite::Result<()> {
    conn.execute_batch("CREATE TABLE IF NOT EXISTS traces (id INTEGER PRIMARY KEY AUTOINCREMENT, session_id TEXT, agent TEXT NOT NULL DEFAULT 'unknown', role TEXT NOT NULL DEFAULT 'store', text TEXT NOT NULL, content_hash TEXT NOT NULL, source_uri TEXT, target_type TEXT, target_id INTEGER, owner_id INTEGER, visibility TEXT, t_event TEXT, t_ingest TEXT NOT NULL DEFAULT (datetime('now'))); CREATE INDEX IF NOT EXISTS idx_traces_target ON traces(target_type, target_id); CREATE INDEX IF NOT EXISTS idx_traces_ingest ON traces(t_ingest); CREATE TABLE IF NOT EXISTS versions (id INTEGER PRIMARY KEY AUTOINCREMENT, op TEXT NOT NULL, summary TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'orphaned')), trace_id INTEGER, target_type TEXT, target_id INTEGER, owner_id INTEGER, created_at TEXT NOT NULL DEFAULT (datetime('now'))); CREATE INDEX IF NOT EXISTS idx_versions_status ON versions(status); CREATE TABLE IF NOT EXISTS head_state (scope TEXT PRIMARY KEY, head_id INTEGER NOT NULL);")?;
    for sql in [
...
pub fn live_version_clause(prefix: &str) -> String {
    format!(
        "({prefix}version_id IS NULL OR {prefix}version_id NOT IN (SELECT id FROM versions WHERE status = 'orphaned'))"
    )
}
...
pub fn record_store_write(
    ...
    let insert_trace = conn.execute("INSERT INTO traces (agent, role, text, content_hash, target_type, target_id, owner_id) VALUES (?1, 'store', ?2, ?3, ?4, ?5, ?6)", params![agent, text, content_hash(text), target_type, target_id, owner_id]);
    ...
    let version = conn.execute("INSERT INTO versions (op, summary, status, trace_id, target_type, target_id, owner_id) VALUES (?1, ?2, 'active', ?3, ?4, ?5, ?6)", params![action, summary_of(action, text), trace_id, target_type, target_id, owner_id]);
    ...
    let _ = set_head(conn, version_id);
    Some(version_id)
}

fn set_head(conn: &Connection, head_id: i64) -> rusqlite::Result<usize> {
    conn.execute("INSERT INTO head_state (scope, head_id) VALUES ('default', ?1) ON CONFLICT(scope) DO UPDATE SET head_id = excluded.head_id", params![head_id])
}
...
pub fn list_versions(conn: &Connection, limit: usize) -> Vec<Value> {
    ...
    let mut stmt = match conn.prepare("SELECT id, op, summary, status, target_type, target_id, created_at FROM versions ORDER BY id DESC LIMIT ?1") { Ok(stmt) => stmt, Err(_) => return Vec::new() };
    stmt.query_map(params![limit as i64], |row| { let id: i64 = row.get(0)?; Ok(json!({"id": id, "op": row.get::<_, String>(1)?, "summary": row.get::<_, String>(2)?, "status": row.get::<_, String>(3)?, "targetType": row.get::<_, Option<String>>(4)?, "targetId": row.get::<_, Option<i64>>(5)?, "createdAt": row.get::<_, String>(6)?, "head": head == Some(id)})) }).map(|rows| rows.filter_map(Result::ok).collect()).unwrap_or_default()
}
