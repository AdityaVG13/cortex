...
fn upsert_memory(conn: &Connection, text: &str, source: &str, mem_type: &str, agent: &str, owner_id: Option<i64>) -> bool {
    ...
    let existing: Option<(i64, String)> = conn
        .query_row("SELECT id, text FROM memories WHERE source = ? AND status = 'active'", [source], |row| Ok((row.get(0)?, row.get(1)?)))
        .ok();
    if let Some((id, existing_text)) = existing {
        if existing_text == text {
            return true;
        }
        let ts = crate::handlers::now_iso();
        let _ = conn.execute("UPDATE memories SET status = 'superseded', valid_until = ?1, updated_at = ?1 WHERE id = ?2", rusqlite::params![ts, id]);
