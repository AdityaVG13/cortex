...
use daemon_lifecycle::{
    daemon_healthy, is_cortex_health_payload, readiness_state_from_payload, validate_spawned_owner_claim, wait_for_health, DAEMON_OWNER_TOKEN_ENV,
...
pub(crate) fn startup_schedule(owner_tag: Option<&str>) -> StartupSchedule {
    ...
    StartupSchedule { index, aging, crystallize, storage_governor_initial }
}
...
pub(crate) async fn startup_single_daemon_preflight(paths: &auth::CortexPaths) -> Result<(), String> {
    ...
    let (mut status, mut body) = match transport::request_url_with_local_ipc_fallback(&client, "GET", &readiness_url, paths, &[], None, Duration::from_secs(2))
        .await
    {
    ...
    loop {
        ...
        match child.try_wait() {
            Ok(Some(status)) => {
                ...
                return Err(format!("daemon spawn exited before health on port {} ({status}): {stderr}", paths.port));
            }
            Ok(None) => {}
        ...
        if started.elapsed() > timeout {
            ...
            let _ = child.wait();
            let stderr = std::fs::read_to_string(&stderr_log_path).unwrap_or_default();
            return Err(format!("daemon spawn started but health is still unavailable on port {}: {stderr}", paths.port));
        }
