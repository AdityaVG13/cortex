...
pub async fn run_daemon(paths: auth::CortexPaths, extra_shutdown: impl std::future::Future<Output = ()> + Send + 'static) {
    ...
    eprintln!(
        "[cortex] Startup scheduling: index={}s, aging={}s, crystallize={}s, storage_governor={}s",
    ...
    {
        let db_clock = state.db.clone();
        let lock_wait = background_lock_wait;
        tokio::spawn(async move {
            if let Some(conn) = acquire_background_db_lock(&db_clock, "clock projection rebuild", lock_wait).await {
                ...
                if decisions > 0 && anchors == 0 {
                    match crate::clockwork::rebuild_clock_projections(&conn, 256) {
                        Ok(n) => eprintln!("[cortex] Clock projections rebuilt on startup ({n} targets)"),
    ...
    }
    {
        ...
        tokio::spawn(async move {
            ...
            if let Some(conn) = acquire_background_db_lock(&db_crystal, "initial crystallization", lock_wait).await {
                let result = crystallize::run_crystallize_pass_with_brain(&conn, crystal_owner_id, &brain_crystal);
            ...
            loop {
                ...
                if let Some(conn) = acquire_background_db_lock(&db_crystal, "crystallization pass", lock_wait).await {
                    crystallize::run_crystallize_pass_with_brain(&conn, crystal_owner_id, &brain_crystal);
                }
            }
        });
    }
