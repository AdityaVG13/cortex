...
pub(crate) fn aggregate_old_feedback(conn: &Connection, failures: &mut Vec<MaintenanceFailure>) -> usize {
    aggregate_old_feedback_with_window(conn, failures, FEEDBACK_AGGREGATION_DAYS)
}
pub(crate) fn aggregate_old_feedback_with_window(conn: &Connection, failures: &mut Vec<MaintenanceFailure>, aggregation_days: i64) -> usize {
...
pub(crate) fn prune_old_benchmark_artifacts(conn: &Connection, failures: &mut Vec<MaintenanceFailure>, retention_days: i64, allow_vacuum: bool) -> usize {
...
pub(crate) fn purge_benchmark_artifacts_with_retention(conn: &Connection, retention_days: Option<i64>, allow_vacuum: bool) -> BenchmarkPurgeResult {
    ...
    exec_batch_counted(conn, &mut result.failures, "benchmark_purge DROP temp table", "DROP TABLE IF EXISTS temp._benchmark_decision_ids;");
    exec_batch_counted(conn, &mut result.failures, "benchmark_purge wal_checkpoint", "PRAGMA wal_checkpoint(TRUNCATE);");
    if allow_vacuum {
        let freelist_pages = freelist_count(conn);
        if freelist_pages > VACUUM_FREELIST_THRESHOLD_PAGES {
            exec_batch_counted(conn, &mut result.failures, "benchmark_purge VACUUM", "VACUUM;");
        }
    }
    result.bytes_after = db_size_bytes(conn);
    if !result.failures.is_empty() {
        eprintln!("[compaction] benchmark purge: {} destructive op(s) FAILED; see FAILED lines above, reported in BenchmarkPurgeResult.failures", result.failures.len());
    }
    result
}
