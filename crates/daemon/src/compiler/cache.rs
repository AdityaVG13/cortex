...
pub(crate) fn cache_get(conn: &Connection, key: &str, expected_hash: &str) -> Option<(String, usize)> {
    let mut stmt = conn.prepare_cached("SELECT compressed, tokens, content_hash FROM context_cache WHERE cache_key = ?1").ok()?;
...
pub(crate) fn build_identity_capsule(conn: &Connection) -> (String, usize) {
    let feedback_hash = {
        ...
        if let Ok(mut stmt) = conn.prepare_cached("SELECT text FROM memories WHERE type = 'feedback' AND status = 'active' AND (expires_at IS NULL OR julianday(expires_at) > julianday('now')) AND (valid_from IS NULL OR julianday(valid_from) <= julianday('now')) AND (valid_until IS NULL OR julianday(valid_until) > julianday('now')) ORDER BY id") {
