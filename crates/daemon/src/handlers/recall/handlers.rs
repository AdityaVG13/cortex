...
fn apply_query_context(ctx: &mut RecallContext, query: &RecallQuery) {
    if let Some(path) = query.path.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        ctx.paths.push(path.to_string());
    }
    if let Some(symbol) = query.symbol.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        ctx.symbols.push(symbol.to_string());
    }
    ctx.goal_id = query.goal_id;
    ctx.session_id = query.session_id.clone();
    ctx.as_of = query.as_of.clone();
}
...
async fn run_unified_recall_handler(
    ...
    source_prefix: Option<String>, agent_default: Option<&str>, missing_q_error: &str, failure_prefix: &str, query: &RecallQuery,
) -> Response {
    ...
    let mut ctx = RecallContext::from_caller(caller_id, state);
    apply_query_context(&mut ctx, query);
...
pub async fn handle_recall_post(State(state): State<RuntimeState>, headers: HeaderMap, Json(body): Json<RecallQuery>) -> Response {
    ...
    run_unified_recall_handler(
        &state,
        &headers,
        body.q.clone().unwrap_or_default(),
        requested_policy_mode,
        body.budget,
        body.k,
        body.source_prefix.clone(),
        body.agent.as_deref(),
        "Missing recall payload field: q",
        "Recall failed",
        &body,
    )
    .await
...
pub async fn handle_as_of(State(state): State<RuntimeState>, Query(query): Query<AsOfQuery>, headers: HeaderMap) -> Response {
    let caller_id = match auth_recall_caller(&headers, &state).await {
        Ok(id) => id,
        Err(resp) => return resp,
    };
    let q = match query.q.as_deref().map(str::trim).filter(|value| !value.is_empty()) {
        Some(value) => value.to_string(),
        None => return json_response(StatusCode::BAD_REQUEST, json!({"error":"Missing query parameter: q"})),
    };
    let raw_t = match query.t.as_deref().map(str::trim).filter(|value| !value.is_empty()) {
        Some(value) => value,
        None => return json_response(StatusCode::BAD_REQUEST, json!({"error":"Missing query parameter: t"})),
    };
    let as_of = match normalize_as_of_timestamp(raw_t) {
        Ok(value) => value,
    ...
    match run_as_of_recall(&conn, &q, &as_of, limit, &ctx) {
        Ok(items) => {
            let results = items.iter().map(|item| item.as_json(&as_of)).collect::<Vec<_>>();
            let payload = json!({"results":results,"count":results.len(),"query":q,"asOf":as_of});
            fire_recall_brain_event(&state, &payload, &agent);
            json_response(StatusCode::OK, payload)
        }
        Err(error) => json_response(StatusCode::INTERNAL_SERVER_ERROR, json!({"error":format!("As-of recall failed: {error}")})),
    }
}
...
pub async fn handle_budget_recall(State(state): State<RuntimeState>, headers: HeaderMap, Query(query): Query<RecallQuery>) -> Response {
    ...
    match run_budget_recall_with_engine(&mut conn, &q, budget, k, &ctx, source_prefix, Some(&state.degraded_mode)) {
        Ok(results) => {
            let usage = compute_recall_budget_usage(&results, budget);
            json_response(
                StatusCode::OK,
                json!({"results":results.into_iter().map(recall_to_json).collect::<Vec<_>>(),"budget":budget,"spent":usage.spent,"saved":usage.saved,"tokenUsageLine":format_recall_token_usage_line(budget,usage),}),
            )
        }
...
pub async fn handle_peek(State(state): State<RuntimeState>, headers: HeaderMap, Query(query): Query<RecallQuery>) -> Response {
    ...
    match run_recall(&mut conn, &q, k, &ctx, source_prefix) {
        Ok(results) => {
            ...
            json_response(
                StatusCode::OK,
                json!({"count":matches.len(),"matches":matches,"tokenUsage":{"used":usage.spent,"saved":usage.saved},"tokenUsageLine":format!("Token usage: used {} tokens, saved {} vs full recall excerpts.",usage.spent,usage.saved)}),
            )
        }
