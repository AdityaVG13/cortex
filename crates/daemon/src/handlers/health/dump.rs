    ...
    let memories: Vec<Value> = conn
        .prepare(
            "SELECT id, text, source, type, tags, source_agent, confidence, status, score, \
             ...
             created_at, updated_at, observed_at, valid_from, valid_until, trust_score \
             FROM memories WHERE status != 'archived' \
             ORDER BY CASE status WHEN 'active' THEN 0 WHEN 'disputed' THEN 1 WHEN 'superseded' THEN 2 ELSE 3 END, score DESC, id DESC",
        )
        .and_then(|mut stmt| {
            stmt.query_map([], |row| {
                Ok(json!({
                    ...
                    "observed_at": row.get::<_, Option<String>>(17).unwrap_or(None),
                    "valid_from": row.get::<_, Option<String>>(18).unwrap_or(None),
                    "valid_until": row.get::<_, Option<String>>(19).unwrap_or(None),
                    "trust_score": row.get::<_, Option<f64>>(20).unwrap_or(Some(0.0)),
                }))
            })
    ...
    let decisions: Vec<Value> = conn
        .prepare(
            "SELECT id, decision, context, type, source_agent, confidence, surprise, status, \
             ...
             confirmed_by, created_at, updated_at, observed_at, valid_from, valid_until, trust_score \
             FROM decisions WHERE status != 'archived' \
             ORDER BY CASE status WHEN 'active' THEN 0 WHEN 'disputed' THEN 1 WHEN 'superseded' THEN 2 ELSE 3 END, score DESC, id DESC",
        )
        .and_then(|mut stmt| {
            stmt.query_map([], |row| {
                Ok(json!({
                    ...
                    "observed_at": row.get::<_, Option<String>>(18).unwrap_or(None),
                    "valid_from": row.get::<_, Option<String>>(19).unwrap_or(None),
                    "valid_until": row.get::<_, Option<String>>(20).unwrap_or(None),
                    "trust_score": row.get::<_, Option<f64>>(21).unwrap_or(Some(0.0)),
                }))
            })
            .map(|rows| rows.filter_map(|r| r.ok()).collect())
        })
