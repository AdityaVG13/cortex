...
pub async fn handle_feedback(State(state): State<RuntimeState>, headers: HeaderMap, Json(body): Json<FeedbackRequest>) -> Response {
    ...
    let frame = crate::clockwork::parse_query_frame(query_text, None, None, None, Vec::new(), Vec::new(), None, None);
    ...
    for source in &body.sources {
        ...
        match conn.execute(
            "INSERT INTO recall_feedback (query_text, query_embedding, result_source, result_type, result_id, signal, agent, query_signature)              VALUES (?1, NULL, ?2, ?3, ?4, ?5, ?6, ?7)",
            params![query_text, source, result_type, result_id, signal, agent, signature],
        ) {
            Ok(_) => stored += 1,
            Err(e) => eprintln!("[feedback] Failed to store for {source}: {e}"),
        }
        if let Some(id) = result_id {
            parsed_targets.push(crate::clockwork::ClockTarget { target_type: result_type, target_id: id });
        }
    }
    if parsed_targets.len() >= 2 {
        for i in 0..parsed_targets.len() {
            for j in (i + 1)..parsed_targets.len() {
                if signal < 0.0 {
                    let _ = crate::clockwork::reject_used_with(&conn, &parsed_targets[i], &parsed_targets[j]);
                } else if signal > 0.0 {
                    let _ = crate::clockwork::record_used_with(&conn, &parsed_targets[i], &parsed_targets[j], None);
                }
            }
        }
    }
