use super::{ClientPermission, mcp_tools, required_permission_for_tool};
...
pub(crate) fn mcp_error_with_data(id: Value, code: i64, message: &str, data: Value) -> Value {
    json!({"jsonrpc":"2.0","id":id,"error":{"code":code,"message":message,"data":data}})
}
pub(crate) fn mcp_resource_uris() -> Vec<&'static str> {
    vec!["cortex://tooling/capabilities", "cortex://tooling/tools"]
}
pub(crate) fn mcp_resources() -> Vec<Value> {
    vec![
        json!({"uri":"cortex://tooling/capabilities","name":"Cortex MCP capabilities","description":"Read-only discovery summary of Cortex tool clusters, permission tiers, and next actions for agents.","mimeType":"application/json"}),
        json!({"uri":"cortex://tooling/tools","name":"Cortex MCP tool catalog","description":"Compact clustered catalog of advertised Cortex MCP tools with required args and permission tier.","mimeType":"application/json"}),
    ]
}
pub(crate) fn mcp_tool_cluster(tool_name: &str) -> &'static str {
    match tool_name {
        "cortex_boot" | "cortex_orient" | "cortex_capabilities" => "orient",
        ...
        "cortex_agent_feedback_record" | "cortex_agent_feedback_stats" | "cortex_feedback" | "cortex_health" | "cortex_digest" | "cortex_lastCall" => {
            "observability"
        }
...
pub(crate) fn tooling_tools_payload() -> Value {
    let tools = mcp_tools()
        ...
        .filter_map(|tool| {
            ...
            let mut parameters = tool.pointer("/inputSchema/properties").and_then(Value::as_object).map(|properties| properties.keys().cloned().collect::<Vec<_>>()).unwrap_or_default();
            parameters.sort();
            Some(json!({"name":name,"cluster":mcp_tool_cluster(&name),"permission":mcp_tool_permission(&name),"description":description,"required":required,"parameters":parameters}))
        })
        .collect::<Vec<_>>();
    json!({"tools":tools,"discovery":{"fullSchemas":"Call tools/list.","capabilities":"Read cortex://tooling/capabilities.","health":"Call cortex_health to confirm daemon liveness and database state."},"commonMistakes":["Use exact tool names from this catalog; aliases are not accepted.","Use read tools before admin or mutation tools when you are unsure of current state.","Do not assume a write/admin tool is available in team mode without a caller-scoped API key."]})
}
pub(crate) fn mcp_resource_payload(uri: &str) -> Option<Value> {
    match uri {
...
pub(crate) fn mcp_resource_read_result(uri: &str, payload: Value) -> Value {
...
pub(crate) fn tool_name_suggestions(provided: &str) -> Vec<String> {
    ...
    let mut candidates: Vec<String> = mcp_tools().into_iter().filter_map(|tool| tool.get("name").and_then(Value::as_str).map(str::to_string)).collect();
    candidates.extend(
        crate::handlers::mcp::legacy_mcp_tools()
            .into_iter()
            .filter_map(|tool| tool.get("name").and_then(Value::as_str).map(str::to_string)),
    );
    if let Some(replacement) = crate::handlers::mcp::removed_tool_replacements().get(&needle).and_then(Value::as_str) {
...
pub(crate) fn payload_token_usage(data: &Value) -> (usize, Option<i64>, Option<usize>) {
    let used = ["spent", "tokenEstimate", "totalTokens", "tokensUsed"]
        .iter()
        .find_map(|key| data.get(key).and_then(Value::as_u64))
        .or_else(|| data.get("savings")?.get("served")?.as_u64())
        .unwrap_or_else(|| estimate_tokens(&data.to_string()) as u64) as usize;
    let saved = data.get("saved").and_then(Value::as_i64).or_else(|| data.get("savings")?.get("saved")?.as_i64());
    let budget = data
        .get("budget")
        .and_then(Value::as_u64)
        .or_else(|| data.get("policy")?.get("budget")?.as_u64())
        .map(|value| value as usize);
    (used, saved, budget)
}
pub(crate) fn token_usage_line(used: usize, saved: Option<i64>, budget: Option<usize>) -> String {
    match (saved, budget) {
        (Some(saved), Some(budget)) if saved >= 0 => format!("Token usage: used {used} tokens, saved {saved} of {budget}."),
        (Some(saved), Some(budget)) => format!("Token usage: used {used} tokens ({} over budget {budget}).", saved.abs()),
        (Some(saved), None) if saved >= 0 => format!("Token usage: used {used} tokens, saved {saved}."),
        (Some(saved), None) => format!("Token usage: used {used} tokens ({} over budget).", saved.abs()),
        (None, Some(budget)) => format!("Token usage: used {used} tokens (budget {budget})."),
        (None, None) => format!("Token usage: used {used} tokens."),
    }
}
pub(crate) fn decorate_tool_payload_with_token_usage(data: Value) -> Value {
    let (used, saved, budget) = payload_token_usage(&data);
    let line = token_usage_line(used, saved, budget);
    match data {
        Value::Object(mut map) => {
            map.entry("tokenUsage".to_string()).or_insert_with(|| json!({"used":used,"saved":saved,"budget":budget}));
