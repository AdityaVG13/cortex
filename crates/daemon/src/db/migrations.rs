...
pub(crate) const SCHEMA_MIGRATIONS: [MigrationDef; 22] = [
...
pub(crate) fn apply_migration_with_logging(conn: &Connection, version: &str, log_success: bool) -> rusqlite::Result<()> {
    match version {
        ...
        "019" => {
            ...
            if table_exists(conn, "traces") && table_exists(conn, "versions") && table_has_column(conn, "decisions", "version_id") {
                Ok(())
            } else {
                Err(migration_error("history migration did not create traces/versions/version_id"))
            }
        }
        "020" => {
            ...
            if table_exists(conn, "entities") && table_exists(conn, "entity_aliases") {
                ...
                Err(migration_error("entity migration did not create entities/entity_aliases"))
            }
        }
        "021" => {
            ensure_column(conn, "memories", "ALTER TABLE memories ADD COLUMN observed_at TEXT")?;
            ...
            conn.execute_batch(
                r#"
                UPDATE memories
                SET observed_at = COALESCE(observed_at, created_at, strftime('%Y-%m-%dT%H:%M:%fZ', 'now')),
                    valid_from = COALESCE(valid_from, observed_at, created_at, strftime('%Y-%m-%dT%H:%M:%fZ', 'now'))
                ...
                UPDATE memories
                SET valid_until = COALESCE(updated_at, observed_at, valid_from)
                WHERE status = 'superseded' AND valid_until IS NULL;

                UPDATE decisions
                SET valid_until = COALESCE(
                    (SELECT MIN(COALESCE(newer.valid_from, newer.observed_at, newer.created_at))
                     FROM decisions newer
                     WHERE newer.supersedes_id = decisions.id),
                    updated_at,
                    observed_at,
                    valid_from
                )
        ...
        "022_clock_anchors" => {
            ...
            let has_sig: bool = conn
                .prepare("PRAGMA table_info(recall_feedback)")
                .ok()
                .and_then(|mut stmt| {
                    stmt.query_map([], |row| row.get::<_, String>(1))
                        .ok()
                        .map(|rows| rows.filter_map(Result::ok).any(|name| name == "query_signature"))
                })
                .unwrap_or(false);
            if !has_sig {
                conn.execute("ALTER TABLE recall_feedback ADD COLUMN query_signature TEXT", [])?;
            }
            Ok(())
        }
