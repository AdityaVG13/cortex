...
fn initialize_with_conn(conn: Connection, paths: &CortexPaths, allow_token_rotation: bool) -> Result<(RuntimeState, oneshot::Receiver<()>), String> {
    ...
    maybe_rebuild_clock_projections(&conn);
    let write_buffer_path = paths.write_buffer.clone();
    let sqlite_vec_canary = SqliteVecCanaryConfig::disabled();
...
fn maybe_rebuild_clock_projections(conn: &Connection) {
    let decisions: i64 = conn.query_row("SELECT COUNT(*) FROM decisions", [], |row| row.get(0)).unwrap_or(0);
    let memories: i64 = conn.query_row("SELECT COUNT(*) FROM memories", [], |row| row.get(0)).unwrap_or(0);
    ...
    if (decisions + memories) > 0 && anchors == 0 {
        match crate::clockwork::rebuild_clock_projections(conn, 256) {
            ...
            Err(err) => eprintln!("[cortex] WARNING: clock projection rebuild failed: {err}"),
        }
    }
}
