...
pub async fn handle_import(State(state): State<RuntimeState>, headers: HeaderMap, Json(payload): Json<ImportPayload>) -> Response {
    ...
    let options = if state.team_mode {
        ...
        ImportOptions { source_agent_fallback: "import-http".to_string(), ..ImportOptions::default() }
    };
    match import_data(&mut conn, &payload, &options) {
        Ok(counts) => json_response(
            StatusCode::OK,
            json!({"imported":{"memories":counts.memories,"decisions":counts.
decisions,}}),
        ),
        Err(detail) => json_response(StatusCode::BAD_REQUEST, json!({"error":"import failed","detail":detail,})),
    }
}
