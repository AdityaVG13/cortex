...
,"profile":profile.clone()}),
    ...
    json_response(
        StatusCode::OK,
        json!({"bootPrompt":result.boot_prompt,"tokenEstimate":result.
...
token_estimate,result.savings.get("saved").and_then(|value|value.as_i64()).unwrap_or(0),max_tokens)}),
    )
}
#[derive(Deserialize, Default)]
...
pub fn query_boot_audits(conn: &rusqlite::Connection, agent: Option<&str>, limit: Option<usize>) -> Result<serde_json::Value, rusqlite::Error> {
    ...
    Ok(json!({"audits":rows,"count":
rows.len(),"retention_days":boot_audit_retention_days(),}))
}
fn row_to_json(row: &rusqlite::Row<'_>) -> rusqlite::Result<serde_json::Value> {
    Ok(json!({"id":row.get::<_,i64>(0)?,"agent":row.get::<_,String>(1)?,"profile":row.get::<_,String>(2)?,"budget_tokens":row.
get::<_,i64>(3)?,"token_estimate":row.get::<_,i64>(4)?,"token_savings":row.get::<_,i64>(5)?,"capsules_count":row.get::<_,i64>(6)?,
"latency_ms":row.get::<_,Option<i64>>(7)?,"created_at":row.get::<_,String>(8)?,}))
}
