...
pub(crate) fn fetch_pending_tasks(conn: &Connection) -> Vec<Value> {
    let mut out = Vec::new();
    if let Ok(mut stmt) = conn.prepare("SELECT task_id, title, priority, project, files_json FROM tasks WHERE status = 'pending' ORDER BY created_at ASC LIMIT 5") {
...
pub(crate) fn fetch_claimed_tasks_for_agent(conn: &Connection, agent: &str) -> Vec<Value> {
    let mut out = Vec::new();
    if let Ok(mut stmt) = conn.prepare("SELECT task_id, title, priority, claimed_at FROM tasks WHERE status = 'claimed' AND claimed_by = ?1 ORDER BY claimed_at ASC") {
        if let Ok(rows) = stmt.query_map(params![agent], |r| {
            Ok(json!({
                "id": r.get::<_, String>(0)?,
                "title": r.get::<_, String>(1)?,
...
pub(crate) fn build_delta_capsule(conn: &Connection, agent: &str) -> (String, usize, String) {
    ...
    if let Ok(mut stmt) = conn.prepare("SELECT id, decision, source_agent, disputes_id FROM decisions WHERE status = 'disputed' ORDER BY created_at DESC LIMIT 6") {
    ...
    if let Some(ref lb) = last_boot {
        if let Ok(mut stmt) = conn.prepare("SELECT decision, context, source_agent FROM decisions WHERE status = 'active' AND created_at >= ?1 ORDER BY created_at DESC LIMIT 5") {
        ...
        if let Ok(mut stmt) = conn.prepare("SELECT text, type FROM memories WHERE status = 'active' AND updated_at >= ?1 AND type != 'state' ORDER BY updated_at DESC LIMIT 3") {
        ...
        if let Ok(mut stmt) = conn.prepare("SELECT type, COUNT(*) as cnt FROM events WHERE created_at > ?1 AND type NOT IN ('brain_init', 'index_all', 'agent_boot') GROUP BY type") {
...
pub(crate) fn estimate_raw_baseline(conn: &Connection, _home: &Path) -> usize {
    ...
    let mem_chars: i64 = conn
        .query_row("SELECT COALESCE(SUM(LENGTH(text)), 0) FROM memories WHERE status = 'active'", [], |r| r.get(0))
        .unwrap_or(0);
    total_chars += mem_chars as usize;
    let dec_chars: i64 = conn
        .query_row("SELECT COALESCE(SUM(LENGTH(decision)), 0) FROM decisions WHERE status = 'active'", [], |r| r.get(0))
        .unwrap_or(0);
...
pub(crate) fn record_boot(conn: &Connection, agent: &str) {
    ...
    let _ = conn.execute(
        ...
        params!["agent_boot", serde_json::to_string(&json!({ "timestamp": &now, "agent": agent })).unwrap_or_default(), agent],
    );
}
