...
fn service_exe_path_from_result(result: std::io::Result<std::path::PathBuf>) -> Result<String, String> {
    result.map(|exe| exe.to_string_lossy().to_string()).map_err(|err| format!("Failed to get exe path: {err}"))
}
...
fn daemon_probe(path: &str) -> Result<(u16, String), String> {
    ...
    let port = crate::auth::CortexPaths::resolve().port;
    let mut stream = std::net::TcpStream::connect_timeout(&std::net::SocketAddr::from(([127, 0, 0, 1], port)), std::time::Duration::from_secs(HEALTH_PROBE_TIMEOUT_SECS))
...
pub fn install() -> bool {
    ...
    match output {
        Ok(o) if o.status.success() => {
            ...
            failure_cmd.args(["/C", &format!("sc.exe failure {} reset= 86400 actions= restart/5000/restart/10000/restart/30000", SERVICE_NAME)]);
