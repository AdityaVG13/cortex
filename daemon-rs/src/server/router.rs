...
pub fn build_router(state: RuntimeState, port: u16) -> Router {
    ...
    Router::new()
        ...
        .route(
            "/recall",
            get(handlers::recall::handle_recall).post(handlers::recall::handle_recall_post),
        )
        .route(
            "/recall/explain",
...
pub(crate) async fn activity_tracking_middleware(
...
pub(crate) async fn handle_mcp_rpc(
    State(state): State<RuntimeState>,
    headers: HeaderMap,
    body: Bytes,
    ...
    let caller_id = match handlers::ensure_auth_with_caller_rated(&headers, &state).await {
        ...
        Err(resp) => {
            ...
            return handlers::json_response(
                status,
                serde_json::json!({"jsonrpc":"2.0","error":{"code":-32600,"message":message,
"hint":hint},"id":serde_json::Value::Null}),
            );
        }
    };
...
,"id":serde_json::Value::Null}),
        ...
        .await
    {
        if !decision.allowed {
            ...
            return handlers::json_response(
                StatusCode::OK,
                serde_json::json!({"jsonrpc":"2.0","error":{"code":-32029,"message":
"budget_exceeded","data":decision.http_body_json()},"id":id}),
            );
        }
    }
