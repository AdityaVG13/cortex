...
async fn daemon_healthy_at(bind: &str, port: u16, expected_paths: Option<&CortexPaths>) -> bool {
    ...
    if let Ok((status, body)) = crate::transport::request_with_local_ipc_fallback(&client, "GET", &base_url, "/readiness", probe_paths, &probe_headers, None, Duration::from_secs(2)).await {
        if let Some(ready) = readiness_state_from_payload(status.as_u16(), &body, Some(port), expected_paths) {
            return ready;
        }
    }
    let (status, body) = match crate::transport::request_with_local_ipc_fallback(&client, "GET", &base_url, "/health", probe_paths, &probe_headers, None, Duration::from_secs(2)).await {
...
mod tests {
    use super::{
        build_owner_token, daemon_owner_signing_key_path, health_probe_base, is_cortex_health_payload, issue_owner_token_for_spawn, load_or_create_owner_signing_key, readiness_state_from_payload,
