...
use super::{
    build_owner_token, daemon_owner_signing_key_path, health_probe_base, is_cortex_health_payload,
    issue_owner_token_for_spawn, load_or_create_owner_signing_key, readiness_state_from_payload,
...
fn cortex_health_payload_rejects_identity_mismatch_for_local_expectations() {
    ...
    let paths =
        CortexPaths::resolve_with_overrides(Some(&home_str), None, Some(7437), Some("127.0.0.1"));
    let valid_body = json!({
        "status": "ok",
        "stats": {
            "memories": 1,
...
fn owner_token_validation_rejects_missing_or_stale_token() {
    ...
    let paths =
        CortexPaths::resolve_with_overrides(Some(&home_str), None, Some(7437), Some("127.0.0.1"));
    let missing =
        validate_spawned_owner_claim(&paths, Some("plugin-claude"), Some(9999), None).unwrap_err();
    ...
    let stale_token = build_owner_token(&key, "plugin-claude", 9999, stale_issued, "stale_nonce")
