use super::read_pool::{open_query_only_connection, read_pool_size_from_env, ReadConnectionPool, ReadConnectionProvider};
use super::runtime::{current_unix_secs, RuntimeState};
use super::types::{BrainFiringEvent, DaemonEvent, SqliteVecCanaryConfig, SqliteVecRouteMode};
...
pub fn initialize(paths: &CortexPaths, allow_token_rotation: bool) -> Result<(RuntimeState, oneshot::Receiver<()>), String> {
    let db_path = &paths.db;
    let conn = crate::db::open(db_path).map_err(|e| format!("Failed to open database at {}: {e}", db_path.display()))?;
    ...
    if crate::db::quick_check(&conn) {
        ...
        eprintln!("[cortex] WARNING: PRAGMA quick_check FAILED on {} -- running full integrity_check", db_path.display());
        let integrity_ok = crate::db::verify_integrity(&conn).unwrap_or(false);
        if !integrity_ok {
            eprintln!("[cortex] WARNING: PRAGMA integrity_check FAILED on {} -- attempting auto-repair", db_path.display());
            drop(conn);
            ...
            match crate::db::auto_repair(db_path, &timestamp) {
                Ok(result) => {
                    ...
                    let conn = crate::db::open(db_path).map_err(|e| format!("Failed to open repaired DB: {e}"))?;
                    crate::db::configure(&conn).map_err(|e| format!("Failed to configure repaired DB: {e}"))?;
                ...
                Err(e) => {
                    ...
                    state.db_corrupted.store(true, std::sync::atomic::Ordering::SeqCst);
                    return Ok((state, rx));
                }
            }
        } else {
...
fn initialize_with_conn(conn: Connection, paths: &CortexPaths, allow_token_rotation: bool) -> Result<(RuntimeState, oneshot::Receiver<()>), String> {
    ...
    let db_read: Arc<dyn ReadConnectionProvider> = Arc::new(ReadConnectionPool::new(read_connections));
    eprintln!(
    ...
    let default_owner_id = if team_mode {
        let from_config = conn
            .query_row("SELECT value FROM config WHERE key = 'owner_user_id' LIMIT 1", [], |row| row.get::<_, String>(0))
            .ok()
    ...
    let token = if team_mode {
        ...
        crate::auth::try_generate_token_for(paths).map_err(|e| format!("Failed to generate shared auth token: {e}"))?
    } else {
        crate::auth::read_token_from(paths).unwrap_or_else(crate::auth::generate_ephemeral_token)
    };
    ...
    if let Some(engine) = embedding_engine.as_ref() {
        eprintln!("[cortex] Embedding engine loaded (model={}, {}-dim, in-process ONNX)", engine.model_key(), engine.dimension());
    } else {
        ...
        match sqlite_vec_canary.route_mode {
            ...
            SqliteVecRouteMode::Trial => {
                if sqlite_vec_canary.trial_percent > 0 {
                    eprintln!("[cortex] sqlite-vec routing mode=trial ({}% sampled)", sqlite_vec_canary.trial_percent);
                } else {
                    eprintln!("[cortex] sqlite-vec routing mode=trial but trial percent is 0 (baseline-only)");
                }
            }
            SqliteVecRouteMode::Primary => {
                eprintln!("[cortex] sqlite-vec routing mode=primary (guarded vec0 routing enabled)");
            }
        }
    }
    ...
    let reranker = if rerank_config.is_active() {
        match crate::rerank::MiniLmReranker::load(&models_dir) {
            ...
            None => {
                eprintln!("[cortex] Reranker unavailable (mode={} requested, missing or invalid assets)", rerank_config.mode.as_str());
                None
            }
        }
    } else {
    ...
    if let Some(error) = budget_config_status.error.as_ref() {
        ...
        eprintln!("[cortex] Budget governance enabled from {}", budget_config_status.source.display());
    }
