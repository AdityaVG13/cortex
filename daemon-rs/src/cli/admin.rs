...
use super::common::{
    admin_request, api_key_output_masked, confirm_action, format_api_key_for_output, json_field, json_str, json_str_or, parse_flag_value, required_cli_positional_or_exit, validate_cli_options_or_exit,
};
use crate::auth;
use crate::budgets;
use crate::db;
...
pub(crate) async fn run_user_cli(paths: &auth::CortexPaths, args: &[String]) {
    ...
    match subcmd {
        "add" => {
            let username = required_cli_positional_or_exit(&args, 3, "Usage: cortex user add <username> [--role member|admin] [--display-name \"...\"]");
    ...
    match subcmd {
        ...
        "add" => {
            ...
            match admin_request(&paths, "POST", "/admin/team/add-member", Some(body)).await {
                Ok(json) => {
                    println!("Added '{}' to team '{}' as {}", json_str(&json, "username"), json_str(&json, "team"), json_str(&json, "role"),);
                }
                Err(e) => {
                    eprintln!("Error: {e}");
                    std::process::exit(1);
