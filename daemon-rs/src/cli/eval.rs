...
pub(crate) fn run_eval_cli(paths: &auth::CortexPaths, args: &[String]) {
    ...
    let signals = snapshot.get("signals").cloned().unwrap_or_else(|| json!({}));
    println!("Eval snapshot ({window_days}d)");
    println!(
        "active: memories={}, decisions={}, open_conflicts={}",
        totals.get("activeMemories").and_then(serde_json::Value::as_i64).unwrap_or(0),
        totals.get("activeDecisions").and_then(serde_json::Value::as_i64).unwrap_or(0),
        totals.get("openConflicts").and_then(serde_json::Value::as_i64).unwrap_or(0)
    );
    println!(
        "window: conflicts={}, resolutions={}, recalls={}",
        window.get("recentConflicts").and_then(serde_json::Value::as_i64).unwrap_or(0),
        window.get("recentResolutions").and_then(serde_json::Value::as_i64).unwrap_or(0),
        window.get("recentRecallQueries").and_then(serde_json::Value::as_i64).unwrap_or(0)
    );
    println!(
        "signals: conflict_burden={:.4}, decay_burden={:.4}, resolution_velocity={:.4}, contradiction_rate={:.4}",
        signals.get("conflictBurden").and_then(serde_json::Value::as_f64).unwrap_or(0.0),
        signals.get("decayBurden").and_then(serde_json::Value::as_f64).unwrap_or(0.0),
        signals.get("resolutionVelocity").and_then(serde_json::Value::as_f64).unwrap_or(0.0),
        signals.get("contradictionRate").and_then(serde_json::Value::as_f64).unwrap_or(0.0)
    );
    println!(
        "task: success_rate={:.4}, first_pass={:.4}, median_time_ms={:.2}, retry_count={:.4}",
        signals.get("taskSuccessRate").and_then(serde_json::Value::as_f64).unwrap_or(0.0),
        signals.get("firstPassSuccess").and_then(serde_json::Value::as_f64).unwrap_or(0.0),
        signals.get("medianTimeToValidResultMs").and_then(serde_json::Value::as_f64).unwrap_or(0.0),
        signals.get("retryCount").and_then(serde_json::Value::as_f64).unwrap_or(0.0)
    );
    println!(
        "memory_quality: stale_hit_rate={:.4}, low_trust_hit_rate={:.4}, consensus_precision={:.4}",
        signals.get("staleMemoryHitRate").and_then(serde_json::Value::as_f64).unwrap_or(0.0),
        signals.get("lowTrustHitRate").and_then(serde_json::Value::as_f64).unwrap_or(0.0),
