...
fn validate_sync_cli_options_or_exit(command: &str, args: &[String]) {
    let result = match command {
        ...
        "watch" => validate_cli_options(args, &["--dir", "--interval-seconds", "--user", "--visibility", "--since", "--cursor-file"], &["--once"]),
...
fn run_sync_import_cli(paths: &auth::CortexPaths, args: &[String]) {
    let parsed = parse_import_cli_args(args, "Usage: cortex sync import --file <path> [--user <username>] [--visibility private|team|shared]");
    let parsed = match parsed {
        Ok(value) => value,
        Err(err) => {
            eprintln!("{err}");
...
fn run_sync_watch_cli(paths: &auth::CortexPaths, args: &[String]) {
    ...
    let Some(dir_raw) = parse_flag_value(args, "--dir") else {
        eprintln!("Usage: cortex sync watch --dir <path> [--interval-seconds <n>] [--once] [--user <username>] [--visibility private|team|shared] [--since <iso>] [--cursor-file <path>]");
        std::process::exit(1);
    };
    ...
    loop {
        ...
        for candidate in candidates {
            ...
            match import_payload_from_file(paths, &import_options, "sync-watch-import", ImportPayloadExpectation::SyncChangeset) {
                Ok(counts) => {
                    eprintln!("[sync watch] imported {} (memories={}, decisions={})", candidate.display(), counts.memories, counts.decisions);
...
fn import_payload_from_file(paths: &auth::CortexPaths, parsed: &ImportCliArgs, source_agent_fallback: &str, expectation: ImportPayloadExpectation) -> Result<export_data::ImportCounts, String> {
    ...
    let payload: export_data::ImportPayload = serde_json::from_value(raw_value).map_err(|e| format!("Import file has unsupported record shape: {e}"))?;
...
fn acquire_sync_lock(paths: &auth::CortexPaths) -> Result<std::fs::File, String> {
    ...
    lock_file.lock_exclusive().map_err(|e| format!("Failed to acquire sync lock {}: {e}", lock_path.display()))?;
    Ok(lock_file)
}
...
fn write_atomic_text_file(path: &Path, contents: &str) -> Result<(), String> {
    ...
    let mut tmp = tempfile::NamedTempFile::new_in(parent).map_err(|e| format!("Failed to create temp file in {}: {e}", parent.display()))?;
    tmp.write_all(contents.as_bytes()).map_err(|e| format!("Failed to write temp file for {}: {e}", path.display()))?;
    tmp.as_file().sync_all().map_err(|e| format!("Failed to flush temp file for {}: {e}", path.display()))?;
    tmp.persist(path).map_err(|e| format!("Failed to replace {} atomically: {}", path.display(), e.error))?;
