...
use super::daemon::{
    background_db_lock_max_wait, build_embeddings_async, count_unembedded_targets_for_model, DEFAULT_EMBED_BACKFILL_BATCH_SIZE, DEFAULT_EMBED_BACKFILL_MAX_BATCHES_PER_PASS,
};
use crate::auth;
use crate::state;
use serde_json::json;
...
pub(crate) async fn run_embeddings_cli(paths: &auth::CortexPaths, args: &[String]) {
    ...
    match subcmd {
        ...
        _ => {
            eprintln!("Usage: cortex embeddings <status|drain> [--json] [--batch-size <n>] [--max-batches <n>] [--lock-wait-ms <n>] [--until-exhausted] [--max-iterations <n>]");
            std::process::exit(1);
        }
    }
}
