...
startup_schedule.storage_governor_initial.as_secs());
    ...
    eprintln!(
        ...
        crate::cli::cleanup::BACKUP_RETENTION_COUNT
    );
    ...
    let _ = cleanup_bridge_backups(&paths.home, schema_version);
    {
        ...
        tokio::spawn(async move {
                ...
                acquire_background_db_lock(&db_index, "startup indexing", lock_wait).await
            {
                ...
                eprintln!(
"[cortex] Startup indexing complete: indexed {indexed}, decayed {decayed} scores in {}ms",started.elapsed().as_millis());
            }
        });
    }
...
queued_total);
                    ...
                    eprintln!("[embeddings] Startup pass left backlog pending; set {}=1 to run a one-time extended drain",
