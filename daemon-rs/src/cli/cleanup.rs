...
const EMBED_BACKFILL_STARTUP_DRAIN_MAX_BATCHES_ENV: &str = "CORTEX_EMBED_BACKFILL_STARTUP_DRAIN_MAX_BATCHES";
    ...
    let mut backups: Vec<_> = std::fs::read_dir(backup_dir)
        .map(|entries| {
            entries
                ...
                .filter(|entry| {
                    entry.file_name().to_string_lossy().starts_with("cortex-") && entry.file_name().to_string_lossy().ends_with(".db") && !entry.file_name().to_string_lossy().contains(".corrupt")
                })
                .collect()
        })
        .unwrap_or_default();
...
pub(crate) fn run_event_compaction_cleanup(db_path: &Path, dry_run: bool, max_passes: usize) -> Result<Vec<String>, String> {
    ...
    for pass in 1..=max_passes.max(1) {
        ...
        lines.push(format!(
            ...
            result.events_pruned, result.benchmark_pruned, result.archived_text_stripped, result.expired_pruned, result.feedback_aggregated, nonboot_before_pass, nonboot_after_pass,
        ));
...
pub(crate) fn run_cleanup_cli(paths: &auth::CortexPaths, dry_run: bool, include_events: bool, max_event_passes: usize) {
    let schema_version = if paths.db.exists() {
        db::open(&paths.db).and_then(|conn| db::current_schema_user_version(&conn)).unwrap_or_default()
    } else {
        0
    };
    let mut lines = Vec::new();
