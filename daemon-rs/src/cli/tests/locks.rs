...
mod tests {
    ...
    fn control_center_lock_detection_reports_cross_process_holder() {
        ...
        let mut child = Command::new(current_exe)
            ...
            .env(CONTROL_CENTER_LOCK_TEST_READY_ENV, ready_file.to_string_lossy().to_string())
    ...
    fn acquire_runtime_lock_rejects_concurrent_startup_burst() {
        ...
        let _global_lock_home = ScopedEnvVar::set("CORTEX_GLOBAL_LOCK_HOME", &global_lock_home_str);
        let home_str = home_dir.to_string_lossy().to_string();
        let paths = Arc::new(auth::CortexPaths::resolve_with_overrides(Some(&home_str), None, Some(7437), None));
    ...
    fn run_stale_pid_cleanup_keeps_lock_file() {
        ...
        assert_eq!(dry_run, vec!["DELETE cortex.pid (process 999999 not running)"]);
        assert!(paths.pid.exists());
        assert!(paths.lock.exists());
        let apply = run_stale_pid_cleanup(&paths, false);
        assert_eq!(apply, vec!["DELETE cortex.pid (process 999999 not running)"]);
        assert!(!paths.pid.exists());
        assert!(paths.lock.exists());
        let _ = fs::remove_dir_all(&home_dir);
    }
    ...
    fn app_managed_startup_heavy_delay_only_applies_to_control_center_owner() {
        ...
        assert_eq!(app_managed_startup_heavy_delay(Some("control-center")), Duration::from_secs(APP_MANAGED_STARTUP_HEAVY_DELAY_SECS));
        assert_eq!(app_managed_startup_heavy_delay(Some("plugin-claude")), Duration::from_secs(0));
        let _startup_delay = ScopedEnvVar::set(APP_MANAGED_STARTUP_DELAY_ENV, "0");
        assert_eq!(app_managed_startup_heavy_delay(Some("control-center")), Duration::from_secs(0));
        drop(_startup_delay);
        let _excessive_delay = ScopedEnvVar::set(APP_MANAGED_STARTUP_DELAY_ENV, "777");
        assert_eq!(app_managed_startup_heavy_delay(Some("control-center")), Duration::from_secs(APP_MANAGED_STARTUP_HEAVY_DELAY_MAX_SECS));
    }
    #[test]
    fn startup_schedule_uses_non_app_defaults_for_plugin_owner() {
        ...
        assert_eq!(schedule.index, Duration::from_secs(DEFAULT_STARTUP_INDEX_DELAY_SECS));
        assert_eq!(schedule.aging, Duration::from_secs(DEFAULT_STARTUP_AGING_DELAY_SECS));
        assert_eq!(schedule.embed, Duration::from_secs(DEFAULT_STARTUP_EMBED_DELAY_SECS));
        assert_eq!(schedule.crystallize, Duration::from_secs(DEFAULT_STARTUP_CRYSTALLIZE_DELAY_SECS));
        assert_eq!(schedule.storage_governor_initial, Duration::from_secs(DEFAULT_STARTUP_STORAGE_GOVERNOR_DELAY_SECS));
    }
    #[test]
    fn startup_schedule_applies_app_managed_offsets_for_control_center() {
        ...
        assert_eq!(schedule.aging, Duration::from_secs(10 + APP_MANAGED_AGING_STARTUP_OFFSET_SECS));
        assert_eq!(schedule.embed, Duration::from_secs(10 + APP_MANAGED_EMBED_STARTUP_OFFSET_SECS));
        assert_eq!(schedule.crystallize, Duration::from_secs(10 + APP_MANAGED_CRYSTALLIZE_STARTUP_OFFSET_SECS));
