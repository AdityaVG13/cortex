...
impl Reranker for NoopReranker {
    ...
    fn rerank(&self, _query: &str, candidates: &[RerankCandidate], fusion_alpha: f64) -> Result<Vec<RerankedScore>, String> {
        let scores = candidates.iter().map(|candidate| (candidate.id.clone(), candidate.base_score as f32)).collect::<Vec<_>>();
...
impl MiniLmReranker {
    ...
    fn try_load(models_dir: &Path) -> Result<Self, String> {
        ...
        let tokenizer = Tokenizer::from_file(&tokenizer_path).map_err(|error| format!("failed to load tokenizer {}: {error}", tokenizer_path.display()))?;
    ...
    fn score_pair(&self, query: &str, document: &str) -> Result<f32, String> {
        ...
        let ids_tensor = Tensor::from_array((shape.clone(), ids[..len].iter().map(|value| *value as i64).collect::<Vec<_>>())).map_err(|error| format!("input_ids tensor failed: {error}"))?;
        let mask_tensor =
            Tensor::from_array((shape.clone(), attention[..len].iter().map(|value| *value as i64).collect::<Vec<_>>())).map_err(|error| format!("attention_mask tensor failed: {error}"))?;
        let type_tensor = Tensor::from_array((shape, type_ids[..len].iter().map(|value| *value as i64).collect::<Vec<_>>())).map_err(|error| format!("token_type_ids tensor failed: {error}"))?;
        ...
        let (_shape, data) = outputs[0].try_extract_tensor::<f32>().map_err(|error| format!("reranker output extraction failed: {error}"))?;
        data.first()
            .copied()
...
pub fn fuse_scores(candidates: &[RerankCandidate], raw_scores: &[(String, f32)], fusion_alpha: f64) -> Vec<RerankedScore> {
    ...
    let raw_by_id = raw_scores.iter().map(|(id, score)| (id.as_str(), *score as f64)).collect::<std::collections::HashMap<_, _>>();
    let base_values = candidates.iter().map(|candidate| candidate.base_score).collect::<Vec<_>>();
    let rerank_values = candidates.iter().map(|candidate| raw_by_id.get(candidate.id.as_str()).copied().unwrap_or(0.0)).collect::<Vec<_>>();
    ...
    let mut fused = candidates
        ...
        .collect::<Vec<_>>();
    fused.sort_by(|(left_idx, left), (right_idx, right)| right.fused_score.partial_cmp(&left.fused_score).unwrap_or(Ordering::Equal).then_with(|| left_idx.cmp(right_idx)));
