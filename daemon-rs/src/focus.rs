...
format!("Failed to start focus: {e}"))?;
...
"Focus closed (no entries captured)"}),
    ...
    if let Some(oid)=owner_id{conn.execute(
...
format!("focus::{label}"),agent],)}.map_err(|e|format!("Failed to store focus summary: {e}"))?;
    conn.execute(
