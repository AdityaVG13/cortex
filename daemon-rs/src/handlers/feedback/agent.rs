...
impl FeedbackAggregate {
    ...
    fn observe(&mut self, outcome: &str, outcome_score: f64, quality_score: f64, age_days: f64, latency_ms: Option<i64>, retries: Option<i64>, tokens_used: Option<i64>) {
        self.count += 1;
        match outcome {
...
pub fn record_agent_feedback_from_value(conn: &Connection, owner_id: i64, args: &Value, fallback_agent: &str) -> Result<Value, String> {
    let outcome = normalize_outcome(arg_value_string(args, &["outcome"]).as_deref().or_else(|| args.get("outcome").and_then(|value| value.as_str())))
...
pub fn build_agent_feedback_stats_payload(conn: &Connection, owner_id: i64, horizon_days: i64, limit: usize, task_class_filter: Option<&str>, agent_filter: Option<&str>) -> Result<Value, String> {
    ...
    for row in rows.flatten() {
        let (agent, task_class, outcome, outcome_score, quality_score, latency_ms, retries, tokens_used, memory_sources_json, age_days) = row;
        ...
        by_task
            ...
            .or_default()
            .observe(&outcome, outcome_score, quality_score, age_days, latency_ms, retries, tokens_used);
        let parsed_sources = memory_sources_json.as_deref().and_then(|raw| serde_json::from_str::<Vec<String>>(raw).ok()).unwrap_or_default();
    ...
    let top_sources: Vec<Value> = top_sources.into_iter().take(10).map(|(source, hits)| json!({ "source": source, "hits": hits })).collect();
