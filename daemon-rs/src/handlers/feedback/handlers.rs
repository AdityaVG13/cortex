...
use super::agent::{build_agent_feedback_stats_payload, normalize_horizon_days, normalize_limit, record_agent_feedback_from_value, AgentFeedbackRecordRequest, AgentFeedbackStatsQuery};
...
pub async fn handle_agent_feedback_stats(State(state): State<RuntimeState>, headers: HeaderMap, Query(query): Query<AgentFeedbackStatsQuery>) -> Response {
    ...
    let conn = state.db.lock().await;
    match build_agent_feedback_stats_payload(&conn, owner_id, horizon_days, limit, query.task_class.as_deref(), query.agent.as_deref()) {
        Ok(payload) => json_response(StatusCode::OK, payload),
        Err(err) => json_error(StatusCode::BAD_REQUEST, &err),
    }
}
