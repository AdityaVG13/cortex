...
use crate::handlers::{
    ensure_auth_with_caller_rated_for_class, ensure_endpoint_budget, estimate_tokens,
    json_response, resolve_source_identity,
};
...
async fn auth_recall_caller(
    headers: &HeaderMap,
    state: &RuntimeState,
) -> Result<Option<i64>, Response> {
    let caller_id =
        ensure_auth_with_caller_rated_for_class(headers, state, RequestClass::Recall).await?;
...
async fn run_unified_recall_handler(
    ...
    match execute_unified_recall(state, q.trim(), budget, k, &agent, &ctx, source_prefix).await {
        ...
        Err(err) => json_response(
            ...
            json!({"error":format!("{failure_prefix}: {err}")}),
        ),
    }
}
