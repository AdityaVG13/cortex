...
#[derive(Clone, Copy)]
enum SearchTableKind { Memories, Decisions }
...
fn search_table_recency(
        ...
        SearchTableKind::Memories => (
            format!(
                "SELECT id, text, source, tags, score, trust_score, retrievals, last_accessed, created_at, compressed_text, age_tier \
                 FROM memories WHERE {ACTIVE_TEMPORAL} \
        ...
        SearchTableKind::Decisions => (
            format!(
                "SELECT id, decision, context, score, trust_score, retrievals, last_accessed, created_at \
                 FROM decisions WHERE {ACTIVE_TEMPORAL} \
    ...
    let rows = stmt.query_map(params![limit as i64, source_like], |row| {
        let effective_score = blend_importance(row.get::<_, Option<f64>>(if use_aging { 4 } else { 3 })?, row.get::<_, Option<f64>>(if use_aging { 5 } else { 4 })?);
        ...
        Ok(SearchCandidate {
            ...
            ts: parse_timestamp_ms(&row.get::<_, Option<String>>(if use_aging { 7 } else { 6 })?.or(row.get::<_, Option<String>>(if use_aging { 8 } else { 7 })?).unwrap_or_default()),
            owner_id: None,
            visibility: None,
        })
...
fn search_table_fts(
  ...
  let sql = match kind {
    SearchTableKind::Memories => format!(
      "SELECT m.id, m.text, m.source, m.tags, m.score, m.trust_score, m.retrievals, m.last_accessed, m.created_at, m.compressed_text, m.age_tier, m.owner_id, m.visibility \
       ...
       WHERE memories_fts MATCH ?1 AND m.{ACTIVE_TEMPORAL} \
    ...
    SearchTableKind::Decisions => format!(
      "SELECT d.id, d.decision, d.context, d.score, d.trust_score, d.retrievals, d.last_accessed, d.created_at, d.compressed_text, d.age_tier, d.owner_id, d.visibility \
       ...
       WHERE decisions_fts MATCH ?1 AND d.{ACTIVE_TEMPORAL} \
  ...
  let mut push_fts_row = |id: i64, primary: String, alt: Option<String>, tags: Option<String>, score: Option<f64>, trust_score: Option<f64>, retrievals: Option<i64>, last_accessed: Option<String>, created_at: Option<String>, compressed_text: Option<String>, age_tier: Option<String>, row_owner_id: Option<i64>, row_visibility: Option<String>| {
    let source_key = search_source_key(kind, id, alt.as_deref());
    if !source_matches_prefix(&source_key, source_prefix) { return; }
    ...
    let haystacks: Vec<String> = match kind {
      SearchTableKind::Memories => vec![primary.to_lowercase(), alt.as_deref().unwrap_or("").to_lowercase(), tags.as_deref().unwrap_or("").to_lowercase()],
  ...
  if matches!(kind, SearchTableKind::Memories) {
    let rows = stmt.query_map(params![fts_query, limit as i64, bm25.memories_text, bm25.memories_source, bm25.memories_tags, source_like], |row| {
      Ok((row.get::<_, i64>(0)?, row.get::<_, String>(1)?, row.get::<_, Option<String>>(2)?, row.get::<_, Option<String>>(3)?, row.get::<_, Option<f64>>(4)?, row.get::<_, Option<f64>>(5)?, row.get::<_, Option<i64>>(6)?, row.get::<_, Option<String>>(7)?, row.get::<_, Option<String>>(8)?, row.get::<_, Option<String>>(9)?, row.get::<_, Option<String>>(10)?, row.get::<_, Option<i64>>(11)?, row.get::<_, Option<String>>(12)?))
    }).map_err(|e| e.to_string())?;
    for row in rows.flatten() {
      let (id, primary, alt, tags, score, trust_score, retrievals, last_accessed, created_at, compressed_text, age_tier, row_owner_id, row_visibility) = row;
      push_fts_row(id, primary, alt, tags, score, trust_score, retrievals, last_accessed, created_at, compressed_text, age_tier, row_owner_id, row_visibility);
    }
  } else {
    let rows = stmt.query_map(params![fts_query, limit as i64, bm25.decisions_text, bm25.decisions_context, source_like], |row| {
      Ok((row.get::<_, i64>(0)?, row.get::<_, String>(1)?, row.get::<_, Option<String>>(2)?, row.get::<_, Option<f64>>(3)?, row.get::<_, Option<f64>>(4)?, row.get::<_, Option<i64>>(5)?, row.get::<_, Option<String>>(6)?, row.get::<_, Option<String>>(7)?, row.get::<_, Option<String>>(8)?, row.get::<_, Option<String>>(9)?, row.get::<_, Option<i64>>(10)?, row.get::<_, Option<String>>(11)?))
    }).map_err(|e| e.to_string())?;
    for row in rows.flatten() {
      ...
      push_fts_row(id, primary, alt, None, score, trust_score, retrievals, last_accessed, created_at, compressed_text, age_tier, row_owner_id, row_visibility);
    }
  }
    ...
    let sql = match kind {
        SearchTableKind::Memories => format!(
            "SELECT id, text, source, tags, score, trust_score, retrievals, last_accessed, created_at \
             FROM memories WHERE {ACTIVE_TEMPORAL} AND (?1 IS NULL OR COALESCE(source, 'memory::' || id) LIKE ?1)"
        ),
        SearchTableKind::Decisions => format!(
            "SELECT id, decision, context, score, trust_score, retrievals, last_accessed, created_at \
             FROM decisions WHERE {ACTIVE_TEMPORAL} AND (?1 IS NULL OR COALESCE(context, 'decision::' || id) LIKE ?1)"
        ),
    };
    let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;
...
fn search_table(
    ...
    let fts_result = search_table_fts(conn, &build_fts_query(&term_groups), limit, source_like.as_deref(), source_prefix, kind, &term_groups, &excerpt_focus_terms, query_text, bm25_weights());
    match fts_result {
        Ok(results) if !results.is_empty() => Ok(results),
        _ => search_table_scan_fallback(conn, query_text, limit, source_prefix, kind, &term_groups, &excerpt_focus_terms, &QueryAlignmentProfile::from_query(query_text)),
    }
}
...
pub(crate) fn search_memories_fallback(conn: &Connection, query_text: &str, limit: usize, source_prefix: Option<&str>) -> Result<Vec<SearchCandidate>, String> {
    let term_groups = build_search_term_groups(query_text);
    search_table_scan_fallback(conn, query_text, limit, source_prefix, SearchTableKind::Memories, &term_groups, &query_focus_terms_for_excerpt(query_text), &QueryAlignmentProfile::from_query(query_text))
}
...
pub(crate) fn search_decisions_fallback(conn: &Connection, query_text: &str, limit: usize, source_prefix: Option<&str>) -> Result<Vec<SearchCandidate>, String> {
    let term_groups = build_search_term_groups(query_text);
    search_table_scan_fallback(conn, query_text, limit, source_prefix, SearchTableKind::Decisions, &term_groups, &query_focus_terms_for_excerpt(query_text), &QueryAlignmentProfile::from_query(query_text))
}
