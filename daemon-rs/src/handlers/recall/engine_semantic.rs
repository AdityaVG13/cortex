    ...
    if let Some(mut stmt) = semantic_memory_stmt {
        if let Ok(rows) = stmt.query_map(
            ...
            ) in rows.flatten()
            {
                ...
                let scaled = scale_semantic_similarity_with_keyword_overlap(sim, &text, &keyword_terms);
    ...
    if let Some(mut stmt) = semantic_decision_stmt {
        if let Ok(rows) = stmt.query_map(
            ...
            ) in rows.flatten()
            {
                ...
                let scaled = scale_semantic_similarity_with_keyword_overlap(sim, &decision, &keyword_terms);
