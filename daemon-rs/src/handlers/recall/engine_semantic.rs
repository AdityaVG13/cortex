...
fn upsert_best_semantic_candidate(candidates: &mut HashMap<String, SemanticCandidate>, source: String, excerpt: String, relevance: f64, importance: f64, ts: i64) {
    let entry = candidates.entry(source.clone()).or_insert(SemanticCandidate { source, excerpt: excerpt.clone(), relevance, importance, ts });
    if relevance > entry.relevance {
        *entry = SemanticCandidate { source: entry.source.clone(), excerpt, relevance, importance, ts };
    }
}
#[derive(Clone, Copy)]
enum SemanticEmbedKind { Memory, Decision }
impl SemanticEmbedKind {
    fn target_type(self) -> &'static str {
        match self { Self::Memory => "memory", Self::Decision => "decision" }
    }
    fn join_table(self) -> &'static str {
        match self { Self::Memory => "memories", Self::Decision => "decisions" }
    }
    fn source_filter(self) -> &'static str {
        match self {
            Self::Memory => "(?3 IS NULL OR m.source LIKE ?3)",
            Self::Decision => "(?3 IS NULL OR d.context LIKE ?3)",
        }
    }
    fn query_with_acl(self) -> String {
        ...
        let cols = match self {
            Self::Memory => "e.vector, m.text, m.source, m.owner_id, m.visibility, m.score, m.trust_score, m.last_accessed, m.created_at",
            Self::Decision => "e.vector, d.decision, d.context, d.owner_id, d.visibility, d.score, d.trust_score, d.last_accessed, d.created_at",
        };
        format!(
            "SELECT {cols} FROM embeddings e JOIN {table} {alias} ON e.target_type = '{tt}' AND e.target_id = {alias}.id AND {alias}.status = 'active' \
             AND ({alias}.expires_at IS NULL OR {alias}.expires_at > datetime('now')) AND ({alias}.valid_from IS NULL OR {alias}.valid_from <= datetime('now')) \
             AND ({alias}.valid_until IS NULL OR {alias}.valid_until > datetime('now')) AND (e.model IS NULL OR LOWER(e.model) = ?1) \
             AND (length(e.vector) = ?2 OR length(e.vector) = ?2/4 + 6) AND {}",
            self.source_filter(), tt = self.target_type()
        )
    }
    fn query_without_acl(self) -> String {
        ...
        let alias = if matches!(self, Self::Memory) { "m" } else { "d" };
        let cols = match self {
            Self::Memory => "e.vector, m.text, m.source, NULL AS owner_id, NULL AS visibility, m.score, m.trust_score, m.last_accessed, m.created_at",
            Self::Decision => "e.vector, d.decision, d.context, NULL AS owner_id, NULL AS visibility, d.score, d.trust_score, d.last_accessed, d.created_at",
        };
        format!(
            "SELECT {cols} FROM embeddings e JOIN {table} {alias} ON e.target_type = '{tt}' AND e.target_id = {alias}.id AND {alias}.status = 'active' \
             AND ({alias}.expires_at IS NULL OR {alias}.expires_at > datetime('now')) AND ({alias}.valid_from IS NULL OR {alias}.valid_from <= datetime('now')) \
...
pub(crate) fn collect_semantic_candidates(conn: &Connection, query_vector: &[f32], query_text: &str, ctx: &RecallContext, source_prefix: Option<&str>) -> Vec<SemanticCandidate> {
    ...
    for kind in [SemanticEmbedKind::Memory, SemanticEmbedKind::Decision] {
        let Some(mut stmt) = prepare_acl_stmt(conn, &kind.query_with_acl(), &kind.query_without_acl()) else { continue };
        let Ok(rows) = stmt.query_map(params![selected_model, expected_vector_bytes, source_like.as_deref()], |row| {
            Ok((row.get::<_, Vec<u8>>(0)?, row.get::<_, String>(1)?, row.get::<_, Option<String>>(2)?, row.get::<_, Option<i64>>(3)?, row.get::<_, Option<String>>(4)?, row.get::<_, Option<f64>>(5)?, row.get::<_, Option<f64>>(6)?, row.get::<_, Option<String>>(7)?, row.get::<_, Option<String>>(8)?))
        }) else { continue };
        for (blob, primary, alt, owner_id, visibility, score, trust_score, last_accessed, created_at) in rows.flatten() {
            if !is_visible(owner_id, visibility.as_deref(), ctx) { continue; }
            let source = match kind {
                SemanticEmbedKind::Memory => primary.clone(),
                SemanticEmbedKind::Decision => decision_source_key(alt, &primary),
            };
            if !source_matches_prefix(&source, source_prefix) { continue; }
            let sim = crate::embeddings::cosine_similarity(query_vector, &crate::embeddings::blob_to_vector(&blob));
            if sim <= semantic_floor as f32 { continue; }
...
pub(crate) fn collect_shadow_semantic_rows(conn: &Connection, ctx: &RecallContext, source_prefix: Option<&str>, expected_dimension: usize) -> Vec<ShadowSemanticRow> {
    let selected_model = crate::embeddings::selected_model_key();
    ...
    for kind in [SemanticEmbedKind::Memory, SemanticEmbedKind::Decision] {
        let Some(mut stmt) = prepare_acl_stmt(conn, &kind.shadow_query_with_acl(), &kind.shadow_query_without_acl()) else { continue };
        let Ok(rows) = stmt.query_map(params![selected_model, expected_vector_bytes, source_like.as_deref()], |row| {
            Ok((row.get::<_, Vec<u8>>(0)?, row.get::<_, String>(1)?, row.get::<_, Option<String>>(2)?, row.get::<_, Option<i64>>(3)?, row.get::<_, Option<String>>(4)?))
        }) else { continue };
        for (blob, primary, alt, owner_id, visibility) in rows.flatten() {
            if !is_visible(owner_id, visibility.as_deref(), ctx) { continue; }
            let source = match kind {
                SemanticEmbedKind::Memory => primary,
                SemanticEmbedKind::Decision => decision_source_key(alt, &primary),
            };
            if !source_matches_prefix(&source, source_prefix) { continue; }
    ...
    let mut rows: Vec<ShadowSemanticRow> = rows_by_source.into_iter().map(|(source, vector)| ShadowSemanticRow { source, vector }).collect();
...
pub(crate) fn vector_to_vec0_literal(vector: &[f32]) -> String {
    ...
    for (idx, value) in vector.iter().enumerate() {
        if idx > 0 { literal.push_str(", "); }
        let _ = write!(&mut literal, "{}", if value.is_finite() { *value } else { 0.0 });
    }
    literal.push(']');
    literal
}
pub(crate) fn run_sqlite_vec_shadow_knn_sources(conn: &Connection, query_vector: &[f32], candidates: &[ShadowSemanticRow], top_k: usize) -> Result<Vec<String>, String> {
    if query_vector.is_empty() || candidates.is_empty() { return Ok(Vec::new()); }
    ...
    let result = (|| -> Result<Vec<String>, String> {
        conn.execute_batch(&format!("DROP TABLE IF EXISTS {SHADOW_TABLE};")).map_err(|err| format!("sqlite-vec shadow drop failed: {err}"))?;
        ...
        for (candidate_idx, candidate) in candidates.iter().enumerate() {
            ...
            insert_stmt.execute(params![candidate_id, vector_to_vec0_literal(&candidate.vector)]).map_err(|err| format!("sqlite-vec shadow insert failed: {err}"))?;
        }
        ...
        let rows = query_stmt.query_map(params![query_literal, k_i64], |row| Ok((row.get::<_, i64>(0)?, row.get::<_, f64>(1)?))).map_err(|err| format!("sqlite-vec shadow query failed: {err}"))?;
        let mut sources = Vec::new();
        let mut seen = HashSet::new();
        for row in rows {
            let (candidate_id, _distance) = row.map_err(|err| format!("sqlite-vec shadow row decode failed: {err}"))?;
            if candidate_id <= 0 { continue; }
            let Some(candidate) = candidates.get((candidate_id - 1) as usize) else { continue };
            if seen.insert(candidate.source.clone()) { sources.push(candidate.source.clone()); }
        }
        Ok(sources)
    })();
...
pub(crate) fn shadow_error_to_unavailable_reason(error: &str) -> Option<&'static str> {
    if error.to_ascii_lowercase().contains("no such module: vec0") { Some("sqlite_vec_not_available") } else { None }
}
pub(crate) fn build_shadow_semantic_explain(conn: &Connection, query_vector: Option<&[f32]>, query_text: &str, ctx: &RecallContext, source_prefix: Option<&str>, top_k: usize, baseline_override: Option<&ShadowSemanticBaseline>) -> Value {
    let top_k = top_k.clamp(1, MAX_SEMANTIC_RRF_CANDIDATES);
    let Some(query_vector) = query_vector else {
        return json!({"enabled": true, "status": "unavailable", "reason": "query_embedding_unavailable", "topK": top_k});
    };
    ...
    let shadow_top_sources = match run_sqlite_vec_shadow_knn_sources(conn, query_vector, &compatible_rows, top_k) {
        ...
        Err(error) => {
            if let Some(reason) = shadow_error_to_unavailable_reason(&error) {
                return json!({"enabled": true, "status": "unavailable", "reason": reason, "detail": error, "topK": top_k, "vectorDimension": vector_dim, "baselineCandidateCount": baseline_candidate_count, "shadowCandidateCount": compatible_count, "baselineTopSources": baseline_top_sources});
            }
            return json!({"enabled": true, "status": "error", "reason": error, "topK": top_k, "vectorDimension": vector_dim, "baselineCandidateCount": baseline_candidate_count, "shadowCandidateCount": compatible_count, "baselineTopSources": baseline_top_sources});
        }
    };
    ...
    let baseline_index: HashMap<&str, usize> = baseline_top_sources.iter().enumerate().map(|(idx, source)| (source.as_str(), idx)).collect();
    ...
    json!({
        "enabled": true, "status": "ok", "topK": top_k, "vectorDimension": vector_dim,
        "baselineCandidateCount": baseline_candidate_count, "shadowCandidateCount": compatible_count,
        "baselineTopSources": baseline_top_sources, "shadowTopSources": shadow_top_sources,
        "overlapCount": overlap_count, "overlapRatio": overlap_ratio, "jaccard": jaccard,
        "matchedRankPairs": matched_rank_pairs, "meanAbsRankDelta": mean_abs_rank_delta, "top1Match": top1_match,
    })
}
