...
impl SemanticEmbedKind {
    ...
    fn query_with_acl(self) -> String {
        ...
        let cols=match self{Self::Memory=>"e.vector, m.text, m.source, m.owner_id, m.visibility, m.score, m.trust_score, m.last_accessed, m.created_at",Self::Decision=>"e.vector, d.decision, d.context, d.owner_id, d.visibility, d.score, d.trust_score, d.last_accessed, d.created_at",};
        format!("SELECT {cols} FROM embeddings e JOIN {table} {alias} ON e.target_type = '{tt}' AND e.target_id = {alias}.id AND {alias}.status = 'active' AND ({alias}.expires_at IS NULL OR {alias}.expires_at > datetime('now')) AND ({alias}.valid_from IS NULL OR {alias}.valid_from <= datetime('now')) AND ({alias}.valid_until IS NULL OR {alias}.valid_until > datetime('now')) AND (e.model IS NULL OR LOWER(e.model) = ?1) AND (length(e.vector) = ?2 OR length(e.vector) = ?2/4 + 6) AND {}",self.source_filter(),tt=self.target_type())
    }
    fn query_without_acl(self) -> String {
        ...
        let cols=match self{Self::Memory=>"e.vector, m.text, m.source, NULL AS owner_id, NULL AS visibility, m.score, m.trust_score, m.last_accessed, m.created_at",Self::Decision=>"e.vector, d.decision, d.context, NULL AS owner_id, NULL AS visibility, d.score, d.trust_score, d.last_accessed, d.created_at",};
        format!("SELECT {cols} FROM embeddings e JOIN {table} {alias} ON e.target_type = '{tt}' AND e.target_id = {alias}.id AND {alias}.status = 'active' AND ({alias}.expires_at IS NULL OR {alias}.expires_at > datetime('now')) AND ({alias}.valid_from IS NULL OR {alias}.valid_from <= datetime('now')) AND ({alias}.valid_until IS NULL OR {alias}.valid_until > datetime('now')) AND (e.model IS NULL OR LOWER(e.model) = ?1) AND (length(e.vector) = ?2 OR length(e.vector) = ?2/4 + 6) AND {}",self.source_filter(),tt=self.target_type())
    }
...
pub(crate) fn shadow_error_to_unavailable_reason(error: &str) -> Option<&'static str> {
    if error.to_ascii_lowercase().contains("no such module: vec0") {
