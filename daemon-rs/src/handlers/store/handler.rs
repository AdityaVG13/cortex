...
use crate::handlers::{ensure_auth_with_caller_rated_for_class, ensure_endpoint_budget, json_response, resolve_source_identity};
...
pub async fn handle_store(State(state): State<RuntimeState>, headers: HeaderMap, Json(body): Json<StoreRequest>) -> Response {
    ...
    let provenance = DecisionProvenance::from_fields(&source_agent, body.source_model.as_deref().or(source_identity.model.as_deref()), body.reasoning_depth.as_deref());
    ...
    match result {
        Ok((entry, new_id)) => {
            if let Some(id) = new_id {
                if let Some(vec) = decision_embedding.as_deref() {
                    if let Err(err) = persist_decision_embedding(&conn, id, vec, embedding_model_key) {
                        eprintln!("[store] Warning: failed to persist decision embedding for {id}: {err}");
                    }
        ...
        Err(StoreError::Internal(err)) => json_response(StatusCode::INTERNAL_SERVER_ERROR, json!({ "error": format!("Store failed: {err}") })),
    }
}
