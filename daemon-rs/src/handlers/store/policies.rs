...
pub(crate) fn handle_contradiction_policy(
    ...
    if incoming_wins {
        if let Some(owner_id) = owner_id {
            tx.execute("UPDATE decisions SET status = 'superseded', updated_at = ?1 WHERE id = ?2 AND owner_id = ?3",
params![ts,existing_id,owner_id],).map_err(|e|StoreError::Internal(e.to_string()))?;
        } else {
            tx.execute(
...
pub(crate) fn handle_refinement_policy(
    ...
    if should_supersede {
        if let Some(owner_id) = owner_id {
            tx.execute("UPDATE decisions SET status = 'superseded', updated_at = ?1 WHERE id = ?2 AND owner_id = ?3",
params![ts,target_id,owner_id],).map_err(|e|StoreError::Internal(e.to_string()))?;
        } else {
            tx.execute(
                "UPDATE decisions SET status = 'superseded', updated_at = ?1 WHERE id = ?2",
                params![ts, target_id],
