...
pub(crate) fn conflict_record_json(record_id: i64, source_decision_id: Option<i64>, target_decision_id: i64, classification: ConflictClassification, status: &str, strategy: Option<&str>) -> Value {
    json!({
        "id": record_id,
...
pub(crate) fn fetch_top_semantic_candidates(conn: &Connection, query_vector: &[f32], owner_id: Option<i64>) -> Result<Vec<SemanticCandidate>, StoreError> {
    ...
    let mut stmt = conn.prepare(sql).map_err(|e| StoreError::Internal(e.to_string()))?;
    let mut candidates = Vec::new();
    if has_owner_scope {
        let rows = stmt
            .query_map([owner_id.unwrap_or_default()], |row| {
                Ok((row.get::<_, i64>(0)?, row.get::<_, String>(1)?, row.get::<_, Option<String>>(2)?, row.get::<_, Vec<u8>>(3)?))
            })
            .map_err(|e| StoreError::Internal(e.to_string()))?;
        for row in rows.flatten() {
            ...
            candidates.push(SemanticCandidate { id, decision, similarity });
        }
    } else {
        let rows = stmt
            .query_map([], |row| {
