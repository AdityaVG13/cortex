...
pub async fn handle_brain_firing_stream(State(state): State<RuntimeState>, Query(query): Query<BrainFiringQuery>) -> Response {
    ...
    let connected = stream::once(async move { Ok::<Event, Infallible>(Event::default().event("connected").data(json!({"type":"connected","timestamp":now_iso()}).to_string())) });
    ...
    let buffered = futures_util::stream::unfold((event_stream, Vec::<BrainFiringEvent>::new(), caller_owner_id), move |(mut events, mut buf, owner)| async move {
    ...
    .filter_map(|item: Option<Value>| async move { item.map(|payload| Ok::<Event, Infallible>(Event::default().event("brain_batch").data(payload.to_string()))) });
