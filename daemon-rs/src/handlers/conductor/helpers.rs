...
pub(crate) fn query_json_rows(
    conn: &rusqlite::Connection, sql: &str, params: &[SqlParam], row_to_json: impl FnMut(&rusqlite::Row<'_>) -> rusqlite::Result<Value>,
...
pub(crate) fn task_row_to_json(row: &rusqlite::Row<'_>) -> rusqlite::Result<Value> {
    Ok(json!({"taskId":row.get::<_,String>(0)?,"title":row.get::<_,String>(1)?,"description":row.get::<_
...
pub(crate) fn fetch_messages_for_agent(conn: &rusqlite::Connection, agent: &str, owner_id: Option<i64>) -> Result<Vec<Value>, String> {
    ...
    query_json_rows(conn, sql, &params_vec, |row| {
        Ok(json!({"id":row.get::<_,String>(0)?,"from":row.get::<_,String>(1)?,
...
pub(crate) fn fetch_tasks(
    conn: &rusqlite::Connection, status_filter: &str, project: Option<&str>, owner_id: Option<i64>, limit: usize, offset: usize,
) -> Result<Vec<Value>, String> {
    let base=
