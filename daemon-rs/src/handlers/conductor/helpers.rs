use super::*;
use crate::handlers::{
    json_response, now_iso, parse_json_array, parse_timestamp_ms, resolve_caller_id,
};
...
pub(crate) fn task_row_to_json(row: &rusqlite::Row<'_>) -> rusqlite::Result<Value> {
    Ok(
        json!({"taskId":row.get::<_,String>(0)?,"title":row.get::<_,String>(1)?,"description":row.get::<_
...
String>>(11)?,"summary":row.get::<_,Option<String>>(12)?}),
    )
}
...
pub(crate) fn clean_expired_locks(
    conn: &rusqlite::Connection,
    owner_id: Option<i64>,
) -> rusqlite::Result<()> {
...
pub(crate) fn clean_old_messages(
    conn: &rusqlite::Connection,
    recipient: &str,
) -> rusqlite::Result<()> {
    conn.execute(
...
pub(crate) fn clean_expired_sessions(
    conn: &rusqlite::Connection,
    owner_id: Option<i64>,
) -> rusqlite::Result<()> {
    if let Some(owner_id) = owner_id {
...
pub(crate) fn last_session_heartbeat_ms(
    conn: &rusqlite::Connection,
    owner_id: Option<i64>,
...
pub(crate) fn should_run_session_freshen(
    conn: &rusqlite::Connection,
    owner_id: Option<i64>,
...
pub(crate) fn run_session_freshen(
    conn: &rusqlite::Connection,
    state: &RuntimeState,
    owner_id: Option<i64>,
) {
...
pub(crate) fn fetch_locks(
    ...
    query_json_rows(conn, sql, &params_vec, |row| {
        Ok(json!({"id":row.get::<_,String>(0)?,"path":row.get::<_,
String>(1)?,"agent":row.get::<_,String>(2)?,"lockedAt":row.get::<_,String>(3)?,"expiresAt":row.get::<_,String>(4)?}))
    })
}
...
![Box::new(owner_id),Box::new(agent.to_string())],)
    } else {
        (
...
to_string())],)
    ...
    query_json_rows(conn, sql, &params_vec, |row| {
        Ok(
            json!({"id":row.get::<_,String>(0)?,"from":row.get::<_,String>(1)?,
"to":row.get::<_,String>(2)?,"message":row.get::<_,String>(3)?,"timestamp":row.get::<_,String>(4)?}),
        )
    })
}
pub(crate) fn fetch_sessions(
    conn: &rusqlite::Connection,
    owner_id: Option<i64>,
) -> Result<Vec<Value>, String> {
