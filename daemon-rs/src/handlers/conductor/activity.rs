...
clone(),agent,description,serde_json::to_string(&files).unwrap_or_else(|_|"[]".to_string()),now_iso(),owner_id],)
    } else {
        conn.
execute("INSERT INTO activities (id, agent, description, files_json, timestamp) VALUES (?1, ?2, ?3, ?4, ?5)",params![id.clone(),
agent,description,serde_json::to_string(&files).unwrap_or_else(|_|"[]".to_string()),now_iso()],)
    };
    match insert {
        Ok(_) => {
...
cutoff.clone())],)
    ...
    let rows=stmt.query_map(rusqlite::params_from_iter(param_refs),|row|{let files:
String=row.get(3)?;Ok(json!({"id":row.get::<_,String>(0)?,"agent":row.get::<_,String>(1)?,"description":row.get::<_,String>(2)?,
"files":parse_json_array(&files),"timestamp":row.get::<_,String>(4)?}))});
    match rows {
        Ok(iter) => {
