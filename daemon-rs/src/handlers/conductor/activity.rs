...
use crate::handlers::{ensure_auth_rated, json_response, now_iso, parse_duration_to_seconds, parse_json_array};
...
pub async fn handle_post_activity(State(state): State<RuntimeState>, headers: HeaderMap, Json(body): Json<ActivityRequest>) -> Response {
    ...
    let insert = if let Some(owner_id) = owner_id {
        conn.execute(
            ...
            params![id.clone(), agent, description, serde_json::to_string(&files).unwrap_or_else(|_| "[]".to_string()), now_iso(), owner_id],
        )
    } else {
        conn.execute(
            "INSERT INTO activities (id, agent, description, files_json, timestamp) VALUES (?1, ?2, ?3, ?4, ?5)",
            params![id.clone(), agent, description, serde_json::to_string(&files).unwrap_or_else(|_| "[]".to_string()), now_iso()],
        )
    };
    match insert {
        Ok(_) => {
        ...
        Err(err) => json_response(StatusCode::INTERNAL_SERVER_ERROR, json!({ "error": format!("Post activity failed: {err}") })),
    }
}
pub async fn handle_get_activity(State(state): State<RuntimeState>, headers: HeaderMap, Query(query): Query<SinceQuery>) -> Response {
    if let Err(resp) = ensure_auth_rated(&headers, &state).await {
        return resp;
    }
    ...
    let mut stmt = match conn.prepare(sql) {
        Ok(stmt) => stmt,
        Err(err) => {
            return json_response(StatusCode::INTERNAL_SERVER_ERROR, json!({ "error": format!("Get activity failed: {err}") }));
        }
    };
    ...
    match rows {
        ...
        Err(err) => json_response(StatusCode::INTERNAL_SERVER_ERROR, json!({ "error": format!("Get activity failed: {err}") })),
    }
}
