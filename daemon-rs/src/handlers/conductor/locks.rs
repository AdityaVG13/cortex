...
pub async fn handle_lock(State(state): State<RuntimeState>, headers: HeaderMap, Json(body): Json<LockRequest>) -> Response {
    ...
    let existing = if let Some(owner_id) = owner_id {
        conn.query_row("SELECT id, agent, expires_at FROM locks WHERE owner_id = ?1 AND path = ?2", params![owner_id, path.clone()], |row| {
    ...
    if let Some((lock_id, holder, holder_expires)) = existing {
        if holder == agent {
            ...
            return json_response(StatusCode::OK, json!({ "locked": true, "lockId": lock_id, "expiresAt": expires_at }));
        }
        let minutes_left = {
    ...
    match insert {
        Ok(_) => {
            checkpoint_wal_best_effort(&conn);
            state.emit("lock", json!({ "action": "acquired", "path": path, "agent": agent }));
            json_response(StatusCode::OK, json!({ "locked": true, "lockId": lock_id, "expiresAt": expires_at }))
        }
        Err(err) => {
...
pub async fn handle_unlock(State(state): State<RuntimeState>, headers: HeaderMap, Json(body): Json<LockRequest>) -> Response {
    ...
    if holder != agent {
        return json_response(StatusCode::FORBIDDEN, json!({ "error": "not_lock_holder", "holder": holder }));
    }
