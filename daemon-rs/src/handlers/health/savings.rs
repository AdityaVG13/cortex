...
"SELECT \
         ...
         GROUP BY day \
         ORDER BY day ASC"
,){Ok(stmt)=>stmt,Err(e)=>return stale_or_error(format!("prepare boot daily query failed: {e}")),};
    ...
    let daily_arr:Vec<Value>=boot_daily_rows.iter().filter_map(|(day,saved,served,baseline,boots)|{if day.
is_empty(){None}else{Some(json!({"date":day,"saved":saved,"served":served,"baseline":baseline,"boots":boots}))}}).collect();
    let
...
"SELECT \
         ...
         WHERE type = 'boot_savings' \
           ...
           AND LOWER(COALESCE(json_extract(data, '$.agent'), '')) NOT LIKE LOWER(?2) \
         GROUP BY agent"
,){Ok(stmt)=>stmt,Err(e)=>return stale_or_error(format!("prepare boot by-agent query failed: {e}")),};
...
"admitted":admitted,"rejected":rejected,"compressionRatio":compression_ratio})}).collect();
...
"SELECT \
            ...
            GROUP BY day \
            ORDER BY day ASC"
,){Ok(stmt)=>stmt,Err(e)=>return stale_or_error(format!("prepare daily savings query failed: {e}")),};
...
"SELECT \
            ...
            WHERE type IN ('boot_savings', 'recall_query', 'store_savings', 'tool_call_savings')
               ...
               AND created_at IS NOT NULL \
             GROUP BY weekday, hour"
,){Ok(stmt)=>stmt,Err(e)=>return stale_or_error(format!("prepare activity heatmap query failed: {e}")),};
