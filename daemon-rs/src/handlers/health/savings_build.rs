...
pub(crate) fn build_shadow_semantic_gate(
    ...
    json!({"ready":ready,"decision":if ready{"ready_for_vec0_trial"}else{"hold"},"target":"sqlite_vec_production_routing",
...
SHADOW_GATE_MAX_MEAN_ABS_RANK_DELTA,"min_top1_match_rate":SHADOW_GATE_MIN_TOP1_MATCH_RATE}})
}
pub(crate) fn build_recall_stats_payload_from_rows(rows: &[(String, String)]) -> Value {
    ...
    for (data_str, created_at) in rows {
        ...
        *tier_counts.entry(tier.clone()).or_insert(0) += 1;
        ...
        recent.push(json!({"timestamp":created_at,"mode":mode,"tier":tier,"budget":
...
"latencyMs":payload.get("latency_ms").and_then(|value|value.as_i64()),}));
    ...
    let tier_distribution: Vec<Value> = tier_counts
        ...
        .map(|(tier, count)| {
            ...
            json!({"tier":tier,"count":count,"percent":percent,"avgLatencyMs":avg_tier_latency})
        })
        .collect();
    ...
    recent.truncate(30);
    json!({"summary":{"totalRecalls":total_recalls,"totalHits":total_hits,"totalBudget":
