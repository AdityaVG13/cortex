...
pub fn list_conflicts_payload(
    ...
    Ok(
        json!({"statusFilter":options.status.as_str(),"classificationFilter":options.
classification,"conflictIdFilter":options.conflict_id,"openCount":open_conflicts.len(),"resolvedCount":resolved_conflicts.len(),
"count":conflicts.len(),"pairs":pairs,"conflicts":conflicts,"conflict":conflict,}),
    )
}
#[allow(clippy::result_large_err)]
pub(crate) fn ensure_admin_surface(
    headers: &HeaderMap,
    state: &RuntimeState,
    conn: &Connection,
...
,params![pattern.clone(),now.clone(),owner_id],).map_err(|e|e.to_string())?;
        let decisions=conn.execute(
...
,params![pattern,now,owner_id],).map_err(|e|e.to_string())?;
        (memories, decisions)
    } else {
        let memories = conn
            .execute(
...
keep_id,other,resolved_at],).map_err(|e|e.to_string())?;
            }
        }
        "merge" => {
            conn.execute(
...
(|e|e.to_string())?;
    ...
    let event_payload = json!({"conflictId":conflict_id,"keepId":keep_id,"winnerId":keep_id,"action":action,
"supersededId":superseded_id,"classification":classification,"similarity":metadata.similarity,"resolvedBy":resolved_by,
"resolvedAt":resolved_at,"notes":metadata.notes,});
    let _ = log_event(
        conn,
        "decision_resolve",
        event_payload.clone(),
    ...
    checkpoint_wal_best_effort(conn);
    Ok(
        json!({"resolved":true,"conflictId":event_payload.get("conflictId").cloned().unwrap_or(Value::
...
.unwrap_or(Value::Null),"notes":event_payload.get("notes").cloned().unwrap_or(Value::Null),}),
    )
}
...
pub async fn handle_permissions_list(
    ...
    match list_permissions(&conn, owner_id) {
        Ok(grants) => json_response(
            StatusCode::OK,
            json!({"ownerId":owner_id,"count":grants.len(),"grants":
grants,}),
        ),
        Err(err) => json_response(
            StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn handle_permissions_grant(
    ...
    let client = if raw_client == "*" {
        ...
        return json_response(
            StatusCode::BAD_REQUEST,
            json!({"error":
"Invalid client id. Use letters, numbers, '-', '_'."}),
        );
    };
        ...
        .map(str::to_string)
    {
        ...
        None => {
            return json_response(
                StatusCode::BAD_REQUEST,
                json!({"error":
"Invalid permission; expected read, write, or admin"}),
            );
        }
    };
    ...
    match grant_permission(&conn, owner_id, &client, &permission, &scope, &granted_by) {
        Ok(()) => json_response(
            StatusCode::OK,
            json!({"granted":true,"ownerId":owner_id,"client":client,"permission":permission,"scope":scope,}),
        ),
        Err(err) => json_response(
            StatusCode::INTERNAL_SERVER_ERROR,
...
"Invalid permission; expected read, write, or admin"}),
    ...
    match revoke_permission(&conn, owner_id, &client, &permission, &scope) {
        Ok(deleted) => json_response(
            StatusCode::OK,
            json!({"revoked":deleted>0,
"deleted":deleted,"ownerId":owner_id,"client":client,"permission":permission,"scope":scope,}),
        ),
        Err(err) => json_response(
            StatusCode::INTERNAL_SERVER_ERROR,
