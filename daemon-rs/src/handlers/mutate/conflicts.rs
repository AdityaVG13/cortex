...
pub fn list_conflicts_payload(conn: &Connection, options: &ConflictListOptions) -> Result<Value, String> {
    ...
    let mut resolved_conflicts =
        if options.status.includes_resolved() { list_resolved_conflicts(conn, options.limit)? } else { Vec::new() };
    ...
    let conflict = if options.conflict_id.is_some() { conflicts.first().cloned().unwrap_or(Value::Null) } else { Value::Null };
    Ok(json!({"statusFilter":options.status.as_str(),"classificationFilter":options.
...
pub fn resolve_decision_with_metadata(
    conn: &mut Connection, keep_id: i64, action: &str, superseded_id: Option<i64>, metadata: ResolutionMetadata,
) -> Result<Value, String> {
...
"resolvedAt":resolved_at,"notes":metadata.notes,});
    ...
    checkpoint_wal_best_effort(conn);
    Ok(json!({"resolved":true,"conflictId":event_payload.get("conflictId").cloned().unwrap_or(Value::
...
pub async fn handle_permissions_grant(
    State(state): State<RuntimeState>, headers: HeaderMap, Json(body): Json<PermissionGrantRequest>,
) -> Response {
    if let Err(resp) = ensure_auth_rated(&headers, &state).await {
        return resp;
    }
...
pub async fn handle_permissions_revoke(
    State(state): State<RuntimeState>, headers: HeaderMap, Json(body): Json<PermissionRevokeRequest>,
) -> Response {
    if let Err(resp) = ensure_auth_rated(&headers, &state).await {
        return resp;
    }
