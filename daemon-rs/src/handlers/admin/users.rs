...
pub async fn handle_user_add(
    ...
    json_response(
        StatusCode::OK,
        json!({"username":username,"user_id":
user_id,"api_key":api_key,"role":role,}),
    )
}
...
pub async fn handle_user_list(State(state): State<RuntimeState>, headers: HeaderMap) -> Response {
    ...
    let users:Vec<serde_json::Value>=match stmt.query_map([],
...
rows)=>rows.flatten().collect(),Err(_)=>Vec::new(),};
