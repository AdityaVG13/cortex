...
pub async fn handle_team_add_member(
    ...
    json_response(
        StatusCode::OK,
        json!({"team":body.team_name,"username":body.username,"role":role,}),
    )
}
pub async fn handle_team_remove_member(
    ...
    json_response(
        StatusCode::OK,
        json!({"removed":{"team":body.team_name,"username":body.username,}}),
    )
}
pub async fn handle_team_list(State(state): State<RuntimeState>, headers: HeaderMap) -> Response {
    ...
    let teams:Vec<serde_json::Value>=match
stmt.query_map([],|row|{Ok(json!({"id":row.get::<_,i64>(0)?,"name":row.get::<_,String>(1)?,"member_count":row.get::<_,i64>(2)?,
"created_at":row.get::<_,Option<String>>(3)?,}))}){Ok(rows)=>rows.flatten().collect(),Err(_)=>Vec::new(),};
