...
"application/json"}),
        json!({"uri":"cortex://tooling/tools","name":"Cortex MCP tool catalog","description":
...
!({"used":used,"saved":saved,"budget":budget})
        ...
        other => {
            json!({"value":other,"tokenUsage":{"used":used,"saved":saved,"budget":budget},"tokenUsageLine":line})
        }
    }
}
pub(crate) fn wrap_mcp_tool_result(_state: &RuntimeState, data: Value) -> Value {
