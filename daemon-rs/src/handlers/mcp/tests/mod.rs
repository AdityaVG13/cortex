...
fn conflict_tools_require_admin_permission_scope() {
    assert_eq!(required_permission_for_tool("cortex_conflicts_list"), Some(ClientPermission::Admin));
    assert_eq!(required_permission_for_tool("cortex_recall"), Some(ClientPermission::Read));
}
#[test]
fn normalize_permission_client_id_strips_parenthetical_suffix() {
    assert_eq!(normalize_permission_client_id("claude(sonnet-4-20250514)"), "claude");
    assert_eq!(normalize_permission_client_id("claude-sonnet-4-20250514"), "claude-sonnet-4-20250514");
}
#[test]
