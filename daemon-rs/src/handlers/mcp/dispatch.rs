...
use super::{
    arg_f64, arg_i64, arg_str, arg_usize, clear_served_scope_for_boot, enforce_client_permission, fetch_last_call, normalize_permission_client_id, parse_client_permission,
    refresh_mcp_session_presence, source_agent_for_tool, source_client_for_permissions, source_model_for_tool, upsert_mcp_session,
    McpPresenceDisposition,
};
...
use crate::handlers::mutate::{
    forget_keyword_scoped, list_conflicts_payload, parse_conflict_id, resolve_decision, resolve_decision_with_metadata, ConflictListOptions, ConflictStatusFilter, ResolutionMetadata,
};
...
pub(crate) async fn mcp_dispatch(state: &RuntimeState, caller_id: Option<i64>, tool_name: &str, args: &Value, source: Option<&SourceIdentity>) -> Result<Value, String> {
    ...
    match tool_name {
        "cortex_boot" => {
            ...
            let raw_agent = arg_str(args, &["agent", "source_agent"]).map(str::to_string).unwrap_or_else(|| source_agent_for_tool(source, "mcp"));
            ...
            crate::handlers::boot::record_boot_audit_best_effort(&conn, &agent, &profile_str, budget, &result, boot_started.elapsed().as_millis() as i64);
        ...
        "cortex_reconnect" => {
            let agent = arg_str(args, &["agent"]).map(str::to_string).unwrap_or_else(|| source_agent_for_tool(source, "mcp"));
            let model = source_model_for_tool(source, args);
        ...
        "cortex_peek" => {
            let query = args.get("query").and_then(|v| v.as_str()).ok_or_else(|| "Missing required argument: query".to_string())?;
        ...
        "cortex_recall" => {
            let query = arg_str(args, &["query", "q"]).ok_or_else(|| "Missing required argument: query".to_string())?;
            let requested_policy_mode = parse_recall_policy_mode(arg_str(args, &["policyMode", "policy_mode"]))?;
            let (budget, mut k, resolved_policy_mode) = resolve_recall_budget_k(requested_policy_mode, arg_usize(args, &["budget", "b"]), arg_usize(args, &["k", "limit"]));
            ...
            if adaptive {
                ...
                if let Some(policy) = recommend_recall_k(&conn, owner_id, agent, task_class, k)? {
                    if let Some(recommended_k) = policy.get("recommendedK").and_then(|value| value.as_u64()) {
            ...
            let ctx = RecallContext::from_caller(caller_id, state);
            let mut payload = execute_unified_recall(state, query, budget, k, agent, &ctx, None).await?;
            if let Value::Object(map) = &mut payload {
                map.insert("policyMode".to_string(), Value::String(resolved_policy_mode.as_str().to_string()));
        ...
        "cortex_recall_policy_explain" => {
            let query = arg_str(args, &["query", "q"]).ok_or_else(|| "Missing required argument: query".to_string())?;
            let requested_policy_mode = parse_recall_policy_mode(arg_str(args, &["policyMode", "policy_mode"]))?;
            let (budget, k, resolved_policy_mode) = resolve_recall_budget_k(requested_policy_mode, arg_usize(args, &["budget", "b"]), arg_usize(args, &["k", "limit"]));
            let pool_k = arg_usize(args, &["pool_k", "poolK", "candidate_pool"]).unwrap_or((k.max(8) * 3).min(64));
            let agent = arg_str(args, &["agent", "source_agent"]).unwrap_or_else(|| source.as_ref().map(|s| s.agent.as_str()).unwrap_or("mcp"));
            let model = source_model_for_tool(source, args);
            let (display_agent, _, disposition) = refresh_mcp_session_presence(state, caller_id, agent, model, "MCP active session").await?;
            if disposition == McpPresenceDisposition::Started {
            ...
            let ctx = RecallContext::from_caller(caller_id, state);
            let mut payload = execute_recall_policy_explain(state, query, budget, k, agent, &ctx, None, pool_k, None).await?;
            if let Value::Object(map) = &mut payload {
                map.insert("policyMode".to_string(), Value::String(resolved_policy_mode.as_str().to_string()));
                if let Some(mode) = requested_policy_mode {
                    map.insert("requestedPolicyMode".to_string(), Value::String(mode.as_str().to_string()));
        ...
        "cortex_store" => {
            ...
            if let (Some(id), Some(vec)) = (new_id, decision_embedding.as_deref()) {
                let model_key = state.embedding_engine.as_ref().map(|engine| engine.model_key()).unwrap_or(crate::embeddings::selected_model_key());
        ...
        "cortex_unfold" => {
                    ...
                    if let Some(id_str) = source.split("::").nth(1) {
                        if let Ok(crystal_id) = id_str.parse::<i64>() {
                            ...
                            let crystal_text = conn
                                .query_row("SELECT consolidated_text FROM memory_clusters WHERE id = ?1", rusqlite::params![crystal_id], |row| {
                                    row.get::<_, String>(0)
                                })
                                .unwrap_or_default();
            ...
            if !found_sources.is_empty() {
                ...
                let query_blob = match state.embedding_engine.clone() {
                    Some(engine) => engine.embed_query_async(query_text.to_string()).await.map(|v| crate::embeddings::vector_to_blob(&v)),
                    None => None,
                };
                let conn = state.db.lock().await;
        ...
        "cortex_forget" => {
            let keyword = args.get("source").and_then(|v| v.as_str()).ok_or_else(|| "Missing required argument: source".to_string())?;
            let mut conn = state.db.lock().await;
        ...
        "cortex_resolve" => {
            let keep_id = args.get("keepId").and_then(|v| v.as_i64()).ok_or_else(|| "Missing required argument: keepId".to_string())?;
            let action = args.get("action").and_then(|v| v.as_str()).ok_or_else(|| "Missing required argument: action".to_string())?;
        ...
        "cortex_conflicts_list" => {
            ...
            let conflict_id = arg_str(args, &["conflictId", "conflict_id", "id"]).map(str::trim).filter(|value| !value.is_empty()).map(str::to_string);
        ...
        "cortex_consensus_promote" => {
            ...
            for conflict in conflicts {
                ...
                let right_score = right
                    ...
                    .unwrap_or(0.0);
                let recommended = conflict.get("trustContext").and_then(|value| value.get("recommendedWinnerId")).and_then(|value| value.as_i64());
        ...
        "cortex_focus_start" => {
            let label = args.get("label").and_then(|v| v.as_str()).ok_or_else(|| "Missing required argument: label".to_string())?;
        ...
        "cortex_permissions_grant" => {
            ...
            let permission = parse_client_permission(permission_raw).ok_or_else(|| "Invalid permission; expected read, write, or admin".to_string())?;
