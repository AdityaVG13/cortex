...
pub fn run_crystallize_pass_with_brain(conn: &Connection, engine: Option<&EmbeddingEngine>, owner_id: Option<i64>, brain: &BrainFiringSender) -> CrystallizeResult {
...
fn generate_cluster_label(members: &[&EmbeddedEntry]) -> String {
    let stop_words: HashSet<&str> = [
        "the", "a", "an", "is", "are", "was", "were", "be", "been", "being", "have", "has", "had", "do", "does", "did", "will", "would", "could", "should", "may", "might", "shall", "can", "to", "of",
        "in", "for", "on", "with", "at", "by", "from", "as", "into", "about", "like", "through", "after", "over", "between", "out", "up", "and", "but", "or", "not", "no", "if", "then", "than",
        "that", "this", "it", "its", "all", "each", "every", "both", "few", "more", "most", "other", "some", "such", "only", "own", "same", "so", "too", "very", "just", "because", "when", "where",
        "how", "what", "which", "who", "whom", "why", "use", "using", "used",
    ]
    .iter()
    .cloned()
    .collect();
...
fn find_existing_crystal(conn: &Connection, label: &str) -> Option<i64> {
    conn.query_row("SELECT id FROM memory_clusters WHERE label = ?1", params![label], |row| row.get(0)).ok()
}
...
pub fn search_crystals_filtered(conn: &Connection, query_vec: &[f32], limit: usize, caller_id: Option<i64>, team_mode: bool) -> Vec<(i64, String, String, f64)> {
    let query_rows = |sql: &str, with_visibility: bool| -> Result<Vec<(i64, Vec<u8>, String, String, Option<i64>, Option<String>)>, rusqlite::Error> {
...
mod tests {
    ...
    fn test_full_crystallize_pass() {
        ...
        for i in 1..=4 {
            ...
            let blob = embeddings::vector_to_blob(&vec);
            conn.execute("INSERT INTO embeddings (target_type, target_id, vector) VALUES ('memory', ?1, ?2)", params![i, blob])
                .unwrap();
        }
