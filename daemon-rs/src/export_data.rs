    ...
    if let Some(memories) = &payload.memories {
        for m in memories {
            let inserted = if memories_has_owner && memories_has_visibility {
                conn.execute(
                    "INSERT INTO memories (text, source, type, tags, source_agent, confidence, score, status, owner_id, visibility)
    ...
    if let Some(decisions) = &payload.decisions {
        for d in decisions {
            let inserted = if decisions_has_owner && decisions_has_visibility {
                conn.execute(
                    "INSERT INTO decisions (decision, context, type, source_agent, confidence, score, status, owner_id, visibility)
