...
pub(crate) const SESSION_RESTART_ATTEMPTS: u32 = 4;
pub(crate) const SESSION_RESTART_DELAY_MS: u64 = 250;
pub(crate) const HEARTBEAT_RECOVERY_FAILURES: u32 = 5;
pub(crate) const STARTUP_IDLE_TIMEOUT_SECS: u64 = 60;
pub(crate) const ORPHAN_CHECK_SECS: u64 = 15;
pub(crate) const MAX_AGENT_HEADER_LEN: usize = 160;
pub(crate) const MAX_MODEL_HEADER_LEN: usize = 160;
pub(crate) const AUTH_TOKEN_CACHE_TTL_MS: u64 = 1_000;
pub(crate) const STDIN_CHANNEL_CAPACITY: usize = 32;
#[derive(Default)]
pub(crate) struct AuthTokenCacheEntry {
...
pub(crate) fn read_auth_token_uncached(token_path: &Path) -> Option<String> {
    match std::fs::read_to_string(token_path) {
        Ok(token) => {
            let trimmed = token.trim();
            if trimmed.is_empty() {
                eprintln!("[cortex-mcp] Auth token file is empty: {}", token_path.display());
                None
            } else {
        ...
        Err(e) => {
            eprintln!("[cortex-mcp] Failed to read auth token {}: {e}", token_path.display());
            None
        }
    }
}
...
pub(crate) fn detect_team_mode(api_key: Option<&str>) -> bool {
    api_key.is_some()
}
pub(crate) fn startup_idle_timeout() -> std::time::Duration {
...
pub(crate) fn normalize_header_value(raw: &str, max_len: usize) -> Option<String> {
    ...
    if trimmed.as_bytes().iter().any(|byte| *byte <= 31 || *byte == 127) {
        return None;
    }
    Some(trimmed.to_string())
}
pub(crate) fn normalize_api_key(api_key: Option<&str>) -> Option<&str> {
    api_key.map(str::trim).filter(|value| !value.is_empty())
}
pub(crate) fn detect_agent_hint(value: &str) -> Option<&'static str> {
...
pub(crate) fn infer_agent_from_process_tree() -> Option<String> {
...
pub(crate) struct ParentProcessRef {
    pid: sysinfo::Pid,
    start_time: u64,
}
pub(crate) fn current_parent_process() -> Option<ParentProcessRef> {
    let mut system = System::new_all();
    system.refresh_processes(ProcessesToUpdate::All, true);
    let current_pid = sysinfo::get_current_pid().ok()?;
...
pub(crate) fn process_is_alive(parent: ParentProcessRef) -> bool {
    let mut system = System::new_all();
    system.refresh_processes(ProcessesToUpdate::Some(&[parent.pid]), true);
    system.process(parent.pid).is_some_and(|process| process.start_time() == parent.start_time)
}
pub(crate) fn resolve_agent_identity(agent_arg: Option<&str>) -> (String, Option<String>) {
    ...
    let agent = match normalize_header_value(&agent, MAX_AGENT_HEADER_LEN) {
        Some(agent) => agent,
        None => {
            eprintln!("[cortex-mcp] Invalid source agent label after normalization; falling back to 'mcp'");
            "mcp".to_string()
        }
    };
    (agent, model)
}
pub(crate) fn local_daemon_base_from_paths(paths: &CortexPaths) -> String {
    crate::transport::local_http_base_url(paths)
}
pub(crate) fn is_local_daemon_base(base_url: &str) -> bool {
...
pub(crate) async fn send_http_over_stream<S>(stream: &mut S, method: &str, path: &str, headers: &[(String, String)], body: Option<&str>) -> Result<(reqwest::StatusCode, String), String>
where
...
pub(crate) fn local_token_fallback_required(base_url: &str, api_key: Option<&str>) -> bool {
    api_key.is_none() && is_local_daemon_base(base_url)
}
pub(crate) fn build_auth_header(base_url: &str, api_key: Option<&str>, allow_local_token_fallback: bool) -> Option<String> {
...
pub(crate) fn requires_explicit_api_key(base_url: &str, api_key: Option<&str>) -> bool {
    api_key.is_none() && !is_local_daemon_base(base_url)
}
...
pub(crate) fn expected_port_from_url(url: &str) -> Option<u16> {
    reqwest::Url::parse(url).ok().and_then(|parsed| parsed.port_or_known_default())
}
pub(crate) fn fallback_health_probe_url(probe_url: &str) -> Option<String> {
    probe_url.strip_suffix("/readiness").map(|base| format!("{base}/health"))
}
pub(crate) fn internal_health_probe_headers() -> [(String, String); 1] {
...
pub(crate) async fn recover_solo_auth(client: &reqwest::Client, health_url: &str, base_url: &str, agent: &str, model: Option<&str>, allow_local_token_fallback: &mut bool) -> bool {
    if !health_check_ready(client, health_url).await {
        *allow_local_token_fallback = false;
        return false;
    }
    *allow_local_token_fallback = true;
    if !session_start_with_retry(client, base_url, None, agent, model, *allow_local_token_fallback).await {
        eprintln!("[cortex-mcp] Auth recovered but session re-registration did not succeed yet");
        return false;
    }
    true
}
pub(crate) async fn session_start_with_retry(client: &reqwest::Client, base_url: &str, api_key: Option<&str>, agent: &str, model: Option<&str>, allow_local_token_fallback: bool) -> bool {
    for attempt in 1..=SESSION_RESTART_ATTEMPTS.max(1) {
        if session_start(client, base_url, api_key, agent, model, allow_local_token_fallback).await {
            return true;
        }
        if attempt < SESSION_RESTART_ATTEMPTS {
            tokio::time::sleep(std::time::Duration::from_millis(SESSION_RESTART_DELAY_MS * attempt as u64)).await;
        }
    }
    false
}
...
#[cfg(unix)]
pub(crate) fn sync_parent_dir(parent: &std::path::Path) -> std::io::Result<()> {
    std::fs::File::open(parent)?.sync_all()
}
#[cfg(not(unix))]
pub(crate) fn sync_parent_dir(_parent: &std::path::Path) -> std::io::Result<()> {
    Ok(())
}
pub(crate) async fn drain_write_buffer(client: &reqwest::Client, base_url: &str, api_key: Option<&str>, agent: &str, model: Option<&str>, paths: &CortexPaths, allow_local_token_fallback: bool) {
...
pub(crate) async fn session_start(client: &reqwest::Client, base_url: &str, api_key: Option<&str>, agent: &str, model: Option<&str>, allow_local_token_fallback: bool) -> bool {
...
pub(crate) enum SessionHeartbeatOutcome {
    Renewed,
    MissingSession,
    Failed,
}
pub(crate) async fn session_heartbeat(client: &reqwest::Client, base_url: &str, api_key: Option<&str>, agent: &str, model: Option<&str>, allow_local_token_fallback: bool) -> SessionHeartbeatOutcome {
    let payload = serde_json::json!({
        "agent": agent,
        "description": model
            .map(|m| format!("MCP session - {m}"))
    ...
    .await
    {
        ...
        Ok((status, _)) if status == reqwest::StatusCode::NOT_FOUND => SessionHeartbeatOutcome::MissingSession,
