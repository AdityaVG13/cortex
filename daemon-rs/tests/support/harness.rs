...
pub fn http_request(port: u16, request: &str) -> Result<String, String> {
    let mut stream = TcpStream::connect(format!("127.0.0.1:{port}")).map_err(|err| err.to_string())?;
    stream
...
fn parse_json_response(method: &str, path: &str, response: &str) -> Result<JsonHttpResponse, String> {
...
fn test_guard(name: &'static str) -> MutexGuard<'static, ()> {
    ...
    for (label, mutex) in registry {
        if *label == name {
            return mutex.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        }
    }
