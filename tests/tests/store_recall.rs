//! Store / recall round-trip: the string we persist must come back as an
//! excerpt, and the decisions row must hold that exact text.
...
async fn store_then_recall_returns_exact_decision_text() {
    ...
    let id = {
        ...
        let persisted: String = conn
            .query_row("SELECT decision FROM decisions WHERE id = ?1", [id], |row| row.get(0))
    ...
    let hit = results.iter().find(|item| item["excerpt"].as_str() == Some(MARKER));
    assert!(
        hit.is_some(),
