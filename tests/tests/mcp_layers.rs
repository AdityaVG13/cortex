//! Runtime-layer isolation: in team mode, a caller must not recall another
//! owner's private decision.
...
async fn team_recall_hides_other_owner_decisions() {
    let state = team_state(1);
    {
        ...
        assert!(id.is_some());
        let owner: i64 = conn
            .query_row("SELECT owner_id FROM decisions WHERE id = ?1", [id.unwrap()], |row| row.get(0))
    ...
    let hidden = execute_unified_recall(
        ...
        8,
        "owner-two",
        &RecallContext { caller_id: Some(2), team_mode: true },
        None,
    )
    .await
    .unwrap_or_else(|err| panic!("recall as owner 2: {err}"));
    let hidden_results = hidden["results"].as_array().unwrap_or_else(|| panic!("results: {hidden}"));
    assert!(
        hidden_results.iter().all(|item| item["excerpt"].as_str() != Some(SECRET)),
    ...
    let visible = execute_unified_recall(
        ...
        8,
        "owner-one",
        &RecallContext { caller_id: Some(1), team_mode: true },
        None,
    )
    .await
    .unwrap_or_else(|err| panic!("recall as owner 1: {err}"));
    let visible_results = visible["results"].as_array().unwrap_or_else(|| panic!("results: {visible}"));
    assert!(
        visible_results.iter().any(|item| item["excerpt"].as_str() == Some(SECRET)),
