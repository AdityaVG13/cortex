...
if [[ -z "${CORTEX_BIN}" ]]; then
  if [[ -f "${REPO_ROOT}/target/release/cortex.exe" ]]; then
    CORTEX_BIN="${REPO_ROOT}/target/release/cortex.exe"
  elif [[ -x "${REPO_ROOT}/target/release/cortex" ]]; then
    CORTEX_BIN="${REPO_ROOT}/target/release/cortex"
  elif [[ -f "${REPO_ROOT}/target/debug/cortex.exe" ]]; then
    CORTEX_BIN="${REPO_ROOT}/target/debug/cortex.exe"
  elif [[ -x "${REPO_ROOT}/target/debug/cortex" ]]; then
    CORTEX_BIN="${REPO_ROOT}/target/debug/cortex"
...
if [[ -z "${CORTEX_BIN}" ]]; then
  echo "FAIL: cortex binary not found (set CORTEX_BIN or build cortex-daemon first)"
  exit 1
fi
