...
check_pattern_absent \
  ...
  "crates/daemon/src/" \
  "No 'self-improvement-engine' in crates/daemon/src" \
  "crates/daemon/src contains 'self-improvement-engine'"

check_pattern_absent \
  "C:\\\\Users\\\\[A-Za-z0-9_.-]+|/Users/[A-Za-z0-9_.-]+" \
  "crates/daemon/src/" \
  "No hardcoded developer home paths in crates/daemon/src" \
  "crates/daemon/src contains hardcoded developer home path(s)"
...
if (cd "${REPO_ROOT}" && "${CARGO_CMD}" clippy -p cortex-daemon -- -D warnings >/dev/null 2>&1); then
  report ok "cargo clippy clean"
else
  report fail "cargo clippy has warnings/errors"
fi

if (cd "${REPO_ROOT}" && "${CARGO_CMD}" test -p cortex-tests >/dev/null 2>&1); then
...
check_pattern_absent \
  ...
  "crates/daemon/src/" \
  "Zero hardcoded knowledge paths in crates/daemon/src" \
  "crates/daemon/src still contains hardcoded knowledge paths"

if grep -q 'index_custom_sources' crates/daemon/src/indexer/mod.rs &&
   grep -q 'sources.toml' crates/daemon/src/indexer/mod.rs; then
