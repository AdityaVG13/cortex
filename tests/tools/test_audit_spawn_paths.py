...
SCRIPT_PATH = Path(__file__).resolve().parents[2] / "tools" / "audit_spawn_paths.py"
...
def _build_fixture_repo(root: Path, *, include_unauthorized_spawn: bool = False) -> None:
    _write(root / "crates" / "daemon" / "src" / "main.rs", "fn allowed() { spawn_daemon(); }\n")
    _write(root / "crates" / "daemon" / "src" / "daemon_lifecycle" / "mod.rs", "fn also_allowed() { try_respawn(); }\n")
    if include_unauthorized_spawn:
        _write(root / "crates" / "daemon" / "src" / "handlers" / "rogue.rs", "fn bad() { spawn_daemon(); }\n")
...
def test_strict_fails_for_unauthorized_spawn_paths(tmp_path: Path) -> None:
    ...
    assert "crates/daemon/src/handlers/rogue.rs" in result.stderr
