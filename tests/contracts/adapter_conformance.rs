...
use support::{
    ...
    wait_for_health, JsonHttpResponse, SpawnTrackedExt, HEALTH_POLL_INTERVAL, STARTUP_TIMEOUT,
};
...
fn health_body_summary(body: &Value) -> Value {
    json!({
        ...
        "retrieval": {
            "engine": body["retrieval"]["engine"],
            "modelFree": body["retrieval"]["modelFree"]
        },
...
fn assert_json_golden(name: &str, actual: &Value) {
    ...
    let expected = fs::read_to_string(&golden_path).unwrap_or_else(|err| {
        panic!(
            "golden file missing: {}\nerror: {err}\nrun with UPDATE_GOLDENS=1 cargo test --test adapter_conformance, then review git diff tests/golden/",
            golden_path.display()
        )
    });
    if actual != expected {
...
fn golden_path(name: &str) -> PathBuf {
    let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("golden");
...
fn generate_contract_coverage_report(spec: &Value) -> String {
    ...
    report.push_str("Generated from `specs/cortex-adapter-contract.yaml` by `tests/contracts/adapter_conformance.rs`.\n\n");
